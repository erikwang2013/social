# Plan de réalisation du jalon M1 en boucle fermée
...
**Objectif :** boucler le jalon M1 : inscription/connexion/profil, publication de post/détail, fil simplifié, likes et commentaires, et générer la documentation d'interface hg/apidoc pour toutes les API de service.

**Architecture :** service (webman) fournit toutes les API HTTP côté client (`/api/v1`) avec authentification JWT à double jeton (access 2 h + refresh 14 j, jti Redis blacklist pour la déconnexion), Eloquent connecté directement à MySQL (préfixe de table `social_`) ; admin assure la gestion des utilisateurs (en M1, requête directe aux tables, gRPC reporté à M2) ; les trois clients mobiles (Android/iOS/HarmonyOS) implémentent en natif connexion + fil + publication de post. Toutes les API de service utilisent les annotations hg/apidoc pour générer la documentation.

**Pile technique :** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**Limites connues (reportées) :** captcha, téléversement d'images, abonnements, fil complet, gRPC admin→service, traduction de contenu → tout en M2+.
...
### Task 1 (backend-service) : couche base de données + squelette de tests

**Fichiers :**
- Créer: `service/config/database.php`
- Créer: `service/database/migrations/m1.sql`
- Créer: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Créer: `service/phpunit.xml`
- Créer: `service/tests/bootstrap.php`
- Créer: `service/tests/AuthModelTest.php`
- Modifier : `service/composer.json` (webman/database + phpunit dev)
...
- [ ] **Step 2 : écrire config/database.php** (sur le modèle de admin/config/database.php : default mysql, connexion de test en sqlite :memory:)
...
- [ ] **Step 3 : écrire m1.sql** (structure uniquement, pour mysql en production/local ; les tests créent les tables en ligne via bootstrap)
...
- [ ] **Step 4 : écrire les modèles** (5 modèles ; Eloquent lit le préfixe depuis config('database'))
...
- [ ] **Step 5 : écrire phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6 : écrire le test tests/AuthModelTest.php**
...
Attendu : 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service) : authentification (inscription/connexion/rafraîchissement/déconnexion/me)

**Fichiers :**
- Créer: `service/app/common/JwtHelper.php`
- Créer: `service/app/middleware/AuthMiddleware.php`
- Créer: `service/app/controller/AuthController.php`
- Créer: `service/config/route.php`
- Créer: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Créer: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Modifier : `service/app/functions.php` (ajouter redisAvailable())

- [ ] **Step 1 : installer le paquet JWT (passer s'il est déjà dans composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2 : configuration jwt.php**
...
- [ ] **Step 3 : ajouter redisAvailable() à app/functions.php** (pour que les tests puissent ignorer les cas dépendant de Redis)
...
> Remarque : JWTFactory::createFromConfig d'erikwang2013/jwt-webman fonctionne aussi ; pour garder les tests simples (sans dépendre de ses entrailles), on utilise directement son socle firebase/php-jwt. Si le paquet ne contient pas firebase/jwt, utiliser la variante `erikwang2013\jwt\JWTFactory` :
...
- [ ] **Step 6 : AuthController** (avec annotations @Apidoc — Task 6 activera la génération apidoc, les écrire dès maintenant)
...
- [ ] **Step 7 : routes config/route.php** (Task 6 ajoutera l'enregistrement apidoc)
...
- [ ] **Step 9 : test tests/AuthFlowTest.php** (les cas dépendant de Redis ne s'exécutent que si redisAvailable())
...
Attendu : tous les tests PASS (cas Redis ignorés ou réussis).
...
### Task 3 (backend-service) : API de profil (mise à jour me)

**Fichiers :**
- Créer: `service/app/controller/MeController.php`
- Créer: `service/tests/MeControllerTest.php`

- [ ] **Step 1 : écrire MeController**
...
- [ ] **Step 2 : écrire MeControllerTest**
...
### Task 4 (backend-service) : posts (publication/fil/détail)

**Fichiers :**
- Créer: `service/app/controller/PostController.php`
- Créer: `service/tests/PostControllerTest.php`

- [ ] **Step 1 : écrire PostController (create/timeline/detail)**
...
- [ ] **Step 2 : écrire PostControllerTest** (le test de tri du fil nécessite usleep(1100000) car created_at a une précision à la seconde)
...
Attendu : tous les tests PASS (test du fil environ 2-3 s).
...
### Task 5 (backend-service) : likes + commentaires

**Fichiers :**
- Modifier : `service/app/controller/PostController.php` (ajouter like/unlike)
- Créer: `service/app/controller/CommentController.php`
- Créer: `service/tests/PostInteractionTest.php`

- [ ] **Step 1 : ajouter like/unlike à PostController**
...
- [ ] **Step 2 : écrire CommentController**
...
- [ ] **Step 3 : écrire PostInteractionTest**
...
### Task 6 (backend-service) : documentation d'interface hg/apidoc

**Fichiers :**
- Créer: `service/config/plugin/hg/apidoc/app.php`
- Créer: `service/config/plugin/hg/apidoc/route.php`
- Créer: `service/app/common/Definitions.php`
- Modifier : `service/config/route.php` (enregistrer les routes apidoc)

- [ ] **Step 1 : installer hg/apidoc** (composer.json contient déjà "hg/apidoc" : "*", un simple update suffit)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2 : écrire la configuration apidoc** (copier le modèle admin et modifier)
...
- [ ] **Step 3 : écrire Definitions.php (structure de réponse commune)**
...
- [ ] **Step 4 : enregistrer les routes apidoc (ajouter à la fin de config/route.php)**
...
- [ ] **Step 5 : vérifier la génération en local**
...
Attendu : sortie `apidoc OK` (titre de page : API client Social).

- [ ] **Step 6 : exécuter tous les tests pour confirmer l'absence de régression**
...
### Task 7 (backend-admin) : gestion des utilisateurs (requête directe aux tables social_)

**Fichiers :**
- Créer: `admin/app/model/SocialUser.php`
- Créer: `admin/app/admin/controller/SocialUserController.php`
- Modifier : `admin/config/route.php` (groupe de routes /admin/user)

- [ ] **Step 1 : écrire le modèle SocialUser**
...
- [ ] **Step 2 : écrire SocialUserController**
...
- [ ] **Step 3 : ajouter les routes admin (fin de config/route.php)**
...
> Remarque : AuthCheck est un middleware existant d'open-admin (si le nom diffère, utiliser le middleware de vérification de connexion existant d'admin). En M2, cela deviendra des appels gRPC vers service.

- [ ] **Step 4 : vérification de syntaxe**
...
Attendu : 4 lignes `No syntax errors detected`.
...
### Task 8 (android-dev) : connexion + fil + publication de post

**Fichiers :**
- Créer: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Créer: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Créer: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- Modifier: `apps/android/app/src/main/java/.../MainActivity.kt`
- Modifier : `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1 : ajouter les dépendances à build.gradle.kts**
...
- [ ] **Step 3 : LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose ou View natives — selon le style UI existant de l'app ; ici implémentation minimale en View natives)
...
### Task 9 (ios-dev) : connexion + fil + publication de post

**Fichiers :**
- Modifier : `apps/ios/.../APIClient.swift` (réécrire)
- Modifier : `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> Remarque : si le projet iOS utilise storyboard/UIKit, remplacer ContentView par un contrôleur UIKit équivalent ; ici, on suit le modèle SwiftUI.
...
### Task 10 (harmonyos-dev) : connexion + fil + publication de post

**Fichiers :**
- Créer: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- Modifier: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1 : ApiService.ets (module http)**
...
- [ ] **Step 2 : réécrire Index.ets (builders loginPanel/timelinePanel)**
...
- [ ] **Step 3 : vérification de syntaxe** (une fois l'environnement DevEco prêt)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Attendu : BUILD SUCCESSFUL (si DevEco/hvigor n'est pas installé en local, passer et vérifier une fois la CI prête).
...
## Vérification finale (après achèvement de toutes les tâches)
...
Valider : `git commit -m "chore: M1 closed-loop complete"` (s'il reste des modifications).

---

## Journal de Self-Review

**1. Couverture du spec (lignes §14 pour M1 + ajouts de l'utilisateur) :**
- Inscription/connexion/profil → T2, T3 ✓
- Publication de post/détail → T4 ✓
- Fil simplifié → T4 ✓
- Likes & commentaires → T5 ✓
- Documentation d'interface hg/apidoc (dernière exigence de l'utilisateur) → T6 ✓ (tous les contrôleurs portent des annotations @Apidoc)
- Les trois clients mobiles implémentent connexion+fil+publication de post → T8/T9/T10 ✓

**2. Recherche de placeholders :** aucun TBD/TODO ; le nom du middleware AuthCheck d'admin précise « si différent, utiliser l'existant », avec un fallback explicite.

**3. Cohérence des types :**
- JwtHelper::encode/decode/revoke/isRevoked sont définis en T2, les tests de T2 les utilisent de manière cohérente
- `$request->uid`/`$request->jti` sont injectés par AuthMiddleware, les tests T3/T4/T5 assignent directement `$req->uid` ✓
- Les champs de réponse `access_token`/`refresh_token`/`expires_in` en snake_case sont définis en T2, les clients T8/T9/T10 s'alignent via SerialName/CodingKeys/littéraux ✓
- PostController::detail/like/unlike et CommentController::index/create injectent `string $id` avec des signatures identiques, la route `{id}` correspond ✓

**4. Limites connues (reportées volontairement) :**
- Les cas Redis sont ignorés sans Redis (redisAvailable())
- gRPC admin→service reporté à M2 (M1 interroge les tables directement, préfixe social_ vérifié)
- Captcha/téléversement d'images/abonnements/fil complet/traduction → M2+
- created_at à la seconde → noté dans le test du fil avec usleep(1100000)
