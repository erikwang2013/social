# خطة تنفيذ مرحلة M1 (الحلقة المغلقة)
...
**الهدف:** إكمال الحلقة المغلقة لـ M1: التسجيل/تسجيل الدخول/الملف الشخصي، نشر المنشور/التفاصيل، الخط الزمني المبسّط، الإعجابات والتعليقات، وإنشاء وثائق الواجهة hg/apidoc لجميع واجهات service.

**البنية:** توفّر خدمة service (webman) جميع واجهات HTTP للعميل (`/api/v1`) مع مصادقة JWT برمزين (access 2 ساعة + refresh 14 يومًا، jti Redis blacklist لتسجيل الخروج)، ويتصل Eloquent مباشرة بـ MySQL (بادئة الجداول `social_`)؛ وتوفر admin إدارة المستخدمين (في M1 استعلام مباشر للجداول، ونقل gRPC مؤجل إلى M2)؛ وتنفّذ تطبيقات الجوال الثلاثة (Android/iOS/HarmonyOS) تسجيل الدخول + الخط الزمني + نشر المنشور بشكل أصلي. تستخدم جميع واجهات service تعليقات hg/apidoc لتوليد وثائق الواجهة.

**حزمة التقنيات:** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**القيود المعروفة (المؤجلة):** رمز التحقق، رفع الصور، المتابعة، الخلاصة الكاملة، gRPC من admin إلى service، ترجمة المحتوى → كلها في M2+.
...
### Task 1 (backend-service): طبقة قاعدة البيانات + هيكل الاختبارات

**الملفات:**
- إنشاء: `service/config/database.php`
- إنشاء: `service/database/migrations/m1.sql`
- إنشاء: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- إنشاء: `service/phpunit.xml`
- إنشاء: `service/tests/bootstrap.php`
- إنشاء: `service/tests/AuthModelTest.php`
- تعديل: `service/composer.json` (webman/database + phpunit dev)

- [ ] **Step 1: تثبيت التبعيات**
...
- [ ] **Step 2: كتابة config/database.php** (على غرار admin/config/database.php: الافتراضي mysql، واتصال الاختبار يستخدم sqlite :memory:)
...
- [ ] **Step 3: كتابة m1.sql** (البنية فقط، لتنفيذ mysql في الإنتاج/محليًا؛ تنشئ الاختبارات الجداول داخليًا عبر bootstrap)
...
- [ ] **Step 4: كتابة النماذج** (5 نماذج؛ يقرأ Eloquent البادئة من config('database'))
...
- [ ] **Step 5: كتابة phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: كتابة الاختبار tests/AuthModelTest.php**
...
متوقع: 3 tests, 3 assertions, PASS.

- [ ] **Step 8: تنفيذ الالتزام**
...
### Task 2 (backend-service): المصادقة (التسجيل/تسجيل الدخول/التحديث/تسجيل الخروج/me)

**الملفات:**
- إنشاء: `service/app/common/JwtHelper.php`
- إنشاء: `service/app/middleware/AuthMiddleware.php`
- إنشاء: `service/app/controller/AuthController.php`
- إنشاء: `service/config/route.php`
- إنشاء: `service/config/plugin/erikwang2013/jwt/jwt.php`
- إنشاء: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- تعديل: `service/app/functions.php` (إضافة redisAvailable())

- [ ] **Step 1: تثبيت حزمة JWT (تخطَّها إن كانت موجودة في composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: إعدادات jwt.php**
...
- [ ] **Step 3: إضافة redisAvailable() إلى app/functions.php** (لتتمكن الاختبارات من تخطي الحالات المرتبطة بـ Redis)
...
> ملاحظة: يمكن استخدام JWTFactory::createFromConfig من erikwang2013/jwt-webman أيضًا؛ ولإبقاء الاختبارات بسيطة (دون الاعتماد على داخلياته)، نستخدم مباشرة طبقة firebase/php-jwt الأساسية. إذا لم تتضمن الحزمة firebase/jwt، فاستخدم بديل `erikwang2013\jwt\JWTFactory`:
...
- [ ] **Step 6: AuthController** (مع تعليقات @Apidoc — ستفعّل Task 6 توليد apidoc، فاكتبها هنا مباشرة)
...
- [ ] **Step 7: المسارات config/route.php** (ستضيف Task 6 تسجيل apidoc)
...
- [ ] **Step 8: الاختبار tests/AuthControllerTest.php**
...
- [ ] **Step 9: الاختبار tests/AuthFlowTest.php** (تُنفَّذ الحالات المرتبطة بـ Redis فقط عند redisAvailable())
...
- [ ] **Step 10: تشغيل الاختبارات**

```bash
cd service && vendor/bin/phpunit
```

متوقع: نجاح جميع الاختبارات PASS (حالات Redis متخطاة أو ناجحة).

- [ ] **Step 11: تنفيذ الالتزام**
...
### Task 3 (backend-service): واجهة الملف الشخصي (تحديث me)

**الملفات:**
- إنشاء: `service/app/controller/MeController.php`
- إنشاء: `service/tests/MeControllerTest.php`

- [ ] **Step 1: كتابة MeController**
...
متوقع: نجاح جميع الاختبارات PASS.

- [ ] **Step 4: تنفيذ الالتزام**
...
### Task 4 (backend-service): المنشورات (النشر/الخط الزمني/التفاصيل)

**الملفات:**
- إنشاء: `service/app/controller/PostController.php`
- إنشاء: `service/tests/PostControllerTest.php`

- [ ] **Step 1: كتابة PostController (create/timeline/detail)**
...
- [ ] **Step 2: كتابة PostControllerTest** (يحتاج اختبار ترتيب الخط الزمني إلى usleep(1100000) لأن created_at بدقة الثواني)
...
متوقع: نجاح جميع الاختبارات PASS (اختبار الخط الزمني نحو 2-3 ثوانٍ).
...
### Task 5 (backend-service): الإعجابات + التعليقات

**الملفات:**
- تعديل: `service/app/controller/PostController.php` (إضافة like/unlike)
- إنشاء: `service/app/controller/CommentController.php`
- إنشاء: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: إضافة like/unlike إلى PostController**
...
- [ ] **Step 2: كتابة CommentController**
...
- [ ] **Step 3: كتابة PostInteractionTest**
...
### Task 6 (backend-service): وثائق الواجهة hg/apidoc

**الملفات:**
- إنشاء: `service/config/plugin/hg/apidoc/app.php`
- إنشاء: `service/config/plugin/hg/apidoc/route.php`
- إنشاء: `service/app/common/Definitions.php`
- تعديل: `service/config/route.php` (تسجيل مسارات apidoc)

- [ ] **Step 1: تثبيت hg/apidoc** (composer.json يحتوي بالفعل على "hg/apidoc": "*"، يكفي تنفيذ update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: كتابة إعدادات apidoc** (نسخ قالب admin وتعديله)
...
- [ ] **Step 3: كتابة Definitions.php (بنية الاستجابة المشتركة)**
...
- [ ] **Step 4: تسجيل مسارات apidoc (إضافة إلى نهاية config/route.php)**
...
- [ ] **Step 5: التحقق من التوليد محليًا**
...
متوقع: إخراج `apidoc OK` (عنوان الصفحة: واجهة عميل Social).

- [ ] **Step 6: تشغيل جميع الاختبارات للتأكد من عدم وجود انحدارات**

```bash
cd service && vendor/bin/phpunit
```

- [ ] **Step 7: تنفيذ الالتزام**
...
### Task 7 (backend-admin): إدارة المستخدمين (استعلام مباشر لجداول social_)

**الملفات:**
- إنشاء: `admin/app/model/SocialUser.php`
- إنشاء: `admin/app/admin/controller/SocialUserController.php`
- تعديل: `admin/config/route.php` (مجموعة مسارات /admin/user)

- [ ] **Step 1: كتابة نموذج SocialUser**
...
- [ ] **Step 2: كتابة SocialUserController**
...
- [ ] **Step 3: إضافة مسارات admin (نهاية config/route.php)**
...
> ملاحظة: AuthCheck هو برمجية وسيطة موجودة في open-admin (إذا اختلف الاسم، استخدم برمجية التحقق من تسجيل الدخول الحالية في admin). سيُستبدل في M2 باستدعاءات gRPC إلى service.
...
متوقع: 4 أسطر من `No syntax errors detected`.
...
### Task 8 (android-dev): تسجيل الدخول + الخط الزمني + نشر المنشور

**الملفات:**
- إنشاء: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- إنشاء: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- إنشاء: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- تعديل: `apps/android/app/src/main/java/.../MainActivity.kt`
- تعديل: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1: إضافة التبعيات إلى build.gradle.kts**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose أو View أصلية — وفق نمط واجهة التطبيق الحالية؛ هنا تنفيذ أدنى بواجهات View أصلية)
...
- [ ] **Step 4: التحقق من الترجمة**

```bash
cd apps/android && ./gradlew assembleDebug
```

متوقع: BUILD SUCCESSFUL.
...
### Task 9 (ios-dev): تسجيل الدخول + الخط الزمني + نشر المنشور

**الملفات:**
- تعديل: `apps/ios/.../APIClient.swift` (إعادة كتابة)
- تعديل: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> ملاحظة: إذا كان مشروع iOS يستخدم storyboard/UIKit، استبدل ContentView بوحدة تحكم UIKit مكافئة؛ هنا نعتمد قالب SwiftUI.

- [ ] **Step 3: تنفيذ الالتزام**
...
### Task 10 (harmonyos-dev): تسجيل الدخول + الخط الزمني + نشر المنشور

**الملفات:**
- إنشاء: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- تعديل: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (وحدة http)**
...
- [ ] **Step 2: إعادة كتابة Index.ets (بُنَاة loginPanel/timelinePanel)**
...
- [ ] **Step 3: التحقق من الصياغة** (بعد جاهزية بيئة DevEco)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

متوقع: BUILD SUCCESSFUL (إذا لم يكن DevEco/hvigor مثبتًا محليًا، تخطَّ والتحقق بعد جاهزية CI).
...
## التحقق النهائي (بعد إنجاز جميع المهام)
...
تنفيذ الالتزام: `git commit -m "chore: M1 closed-loop complete"` (إذا وُجدت تغييرات متبقية).

---

## سجل المراجعة الذاتية

**1. تغطية المواصفات (أسطر §14 لـ M1 + إضافات المستخدم):**
- التسجيل/تسجيل الدخول/الملف الشخصي → T2, T3 ✓
- نشر المنشور/التفاصيل → T4 ✓
- الخط الزمني المبسّط → T4 ✓
- الإعجابات والتعليقات → T5 ✓
- وثائق الواجهة hg/apidoc (أحدث متطلبات المستخدم) → T6 ✓ (جميع وحدات التحكم تحمل تعليقات @Apidoc)
- تطبيقات الجوال الثلاثة تنفذ تسجيل الدخول+الخط الزمني+نشر المنشور → T8/T9/T10 ✓

**2. فحص العناصر النائبة:** لا يوجد TBD/TODO؛ اسم برمجية AuthCheck الوسيطة في admin مُعلَّم بأنه "إذا اختلف الاسم فاستخدم الموجود"، مع بديل fallback واضح.

**3. اتساق الأنواع:**
- تُعرَّف JwtHelper::encode/decode/revoke/isRevoked في T2 وتستخدمها اختبارات T2 باستمرار
- يحقن AuthMiddleware كلاً من `$request->uid`/`$request->jti`، وتُسند اختبارات T3/T4/T5 القيمة مباشرة إلى `$req->uid` ✓
- حروف الاستجابة `access_token`/`refresh_token`/`expires_in` بنمط snake_case معرّفة في T2، وتطابِقها عملاء T8/T9/T10 عبر SerialName/CodingKeys/القيم الحرفية ✓
- تحقن PostController::detail/like/unlike و CommentController::index/create `string $id` بتوقيعات متطابقة، ومسار `{id}` يقابلها ✓

**4. القيود المعروفة (المؤجلة عمدًا):**
- تُتخطى حالات Redis عند غياب Redis (redisAvailable())
- نقل gRPC من admin إلى service مؤجل إلى M2 (M1 يستعلم الجداول مباشرة، وبادئة social_ موثقة)
- رمز التحقق/رفع الصور/المتابعة/الخلاصة الكاملة/الترجمة → M2+
- دقة created_at بالثواني → موثقة في اختبار الخط الزمني عبر usleep(1100000)
