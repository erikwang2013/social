# M1 ক্লোজড-লুপ মাইলস্টোন বাস্তবায়ন পরিকল্পনা
...
**লক্ষ্য:** M1 ক্লোজড-লুপ সম্পূর্ণ করুন: রেজিস্ট্রেশন/লগইন/প্রোফাইল, পোস্ট তৈরি/বিস্তারিত, সরলীকৃত টাইমলাইন, লাইক ও মন্তব্য, এবং service-এর সব API-র জন্য hg/apidoc ইন্টারফেস ডকুমেন্টেশন তৈরি।

**আর্কিটেকচার:** service (webman) সব ক্লায়েন্ট-সাইড HTTP API (`/api/v1`) সরবরাহ করে, JWT ডুয়াল-টোকেন অথেনটিকেশন (access 2ঘ + refresh 14দ, লগআউটের জন্য jti Redis ব্ল্যাকলিস্ট), Eloquent সরাসরি MySQL-এর সাথে সংযুক্ত (টেবিল প্রিফিক্স `social_`); admin ব্যবহারকারী ব্যবস্থাপনা সরবরাহ করে (M1-এ সরাসরি টেবিল কোয়েরি, gRPC M2-তে স্থগিত); তিনটি মোবাইল ক্লায়েন্ট (Android/iOS/HarmonyOS) নেটিভভাবে লগইন + টাইমলাইন + পোস্ট তৈরি বাস্তবায়ন করে। সব service API ইন্টারফেস ডকুমেন্টেশন তৈরি করতে hg/apidoc অ্যানোটেশন ব্যবহার করে।

**টেক স্ট্যাক:** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**জ্ঞাত সীমাবদ্ধতা (স্থগিত):** ক্যাপচা, ছবি আপলোড, ফলো, সম্পূর্ণ ফিড, admin→service gRPC, কন্টেন্ট অনুবাদ → সব M2+।
...
### Task 1 (backend-service): ডেটাবেস স্তর + টেস্ট কাঠামো

**ফাইল:**
- তৈরি করুন: `service/config/database.php`
- তৈরি করুন: `service/database/migrations/m1.sql`
- তৈরি করুন: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- তৈরি করুন: `service/phpunit.xml`
- তৈরি করুন: `service/tests/bootstrap.php`
- তৈরি করুন: `service/tests/AuthModelTest.php`
- পরিবর্তন করুন: `service/composer.json` (webman/database + phpunit dev)

- [ ] **Step 1: ডিপেন্ডেন্সি ইনস্টল করুন**
...
- [ ] **Step 2: config/database.php লিখুন** (admin/config/database.php-এর অনুকরণে: default mysql, টেস্ট সংযোগ sqlite :memory: ব্যবহার করে)
...
- [ ] **Step 3: m1.sql লিখুন** (শুধু কাঠামো, প্রোডাকশন/লোকাল mysql-এর জন্য; টেস্ট bootstrap দিয়ে ইনলাইন টেবিল তৈরি করে)
...
- [ ] **Step 4: মডেলগুলো লিখুন** (৫টি মডেল; Eloquent config('database') থেকে prefix পড়ে)
...
- [ ] **Step 5: phpunit.xml + tests/bootstrap.php লিখুন**
...
- [ ] **Step 6: টেস্ট tests/AuthModelTest.php লিখুন**
...
প্রত্যাশিত: 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service): অথেনটিকেশন (রেজিস্টার/লগইন/রিফ্রেশ/লগআউট/me)

**ফাইল:**
- তৈরি করুন: `service/app/common/JwtHelper.php`
- তৈরি করুন: `service/app/middleware/AuthMiddleware.php`
- তৈরি করুন: `service/app/controller/AuthController.php`
- তৈরি করুন: `service/config/route.php`
- তৈরি করুন: `service/config/plugin/erikwang2013/jwt/jwt.php`
- তৈরি করুন: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- পরিবর্তন করুন: `service/app/functions.php` (redisAvailable() যোগ করুন)

- [ ] **Step 1: JWT প্যাকেজ ইনস্টল করুন (composer.json-এ থাকলে বাদ দিন)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: jwt.php কনফিগারেশন**
...
- [ ] **Step 3: app/functions.php-এ redisAvailable() যোগ করুন** (যাতে টেস্টগুলো Redis-নির্ভর কেস বাদ দিতে পারে)
...
> নোট: erikwang2013/jwt-webman-এর JWTFactory::createFromConfig-ও ব্যবহার করা যায়; টেস্ট সহজ রাখতে (এর অভ্যন্তরীণ কাঠামোর ওপর নির্ভর না করে) সরাসরি এর অন্তর্নিহিত firebase/php-jwt ব্যবহার করা হয়। প্যাকেজে firebase/jwt না থাকলে `erikwang2013\jwt\JWTFactory` বিকল্প ব্যবহার করুন:
...
- [ ] **Step 6: AuthController** (@Apidoc অ্যানোটেশনসহ — Task 6 apidoc জেনারেশন সক্রিয় করবে, এখানে সরাসরি লিখে রাখুন)
...
- [ ] **Step 7: রাউট config/route.php** (Task 6 apidoc register যোগ করবে)
...
- [ ] **Step 8: টেস্ট tests/AuthControllerTest.php**
...
- [ ] **Step 9: টেস্ট tests/AuthFlowTest.php** (Redis-নির্ভর কেস শুধু redisAvailable() হলে চলে)
...
প্রত্যাশিত: সব PASS (Redis কেস বাদ পড়েছে বা পাস করেছে)।
...
### Task 3 (backend-service): প্রোফাইল API (me আপডেট)

**ফাইল:**
- তৈরি করুন: `service/app/controller/MeController.php`
- তৈরি করুন: `service/tests/MeControllerTest.php`

- [ ] **Step 1: MeController লিখুন**
...
### Task 4 (backend-service): পোস্ট (তৈরি/টাইমলাইন/বিস্তারিত)

**ফাইল:**
- তৈরি করুন: `service/app/controller/PostController.php`
- তৈরি করুন: `service/tests/PostControllerTest.php`

- [ ] **Step 1: PostController লিখুন (create/timeline/detail)**
...
- [ ] **Step 2: PostControllerTest লিখুন** (টাইমলাইন সাজানোর টেস্টে usleep(1100000) দরকার, কারণ created_at-এর নির্ভুলতা সেকেন্ড-স্তরের)
...
প্রত্যাশিত: সব PASS (টাইমলাইন টেস্ট প্রায় ২-৩ সেকেন্ড)।
...
### Task 5 (backend-service): লাইক + মন্তব্য

**ফাইল:**
- পরিবর্তন করুন: `service/app/controller/PostController.php` (like/unlike যোগ করুন)
- তৈরি করুন: `service/app/controller/CommentController.php`
- তৈরি করুন: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: PostController-এ like/unlike যোগ করুন**
...
- [ ] **Step 2: CommentController লিখুন**
...
- [ ] **Step 3: PostInteractionTest লিখুন**
...
### Task 6 (backend-service): hg/apidoc ইন্টারফেস ডকুমেন্টেশন

**ফাইল:**
- তৈরি করুন: `service/config/plugin/hg/apidoc/app.php`
- তৈরি করুন: `service/config/plugin/hg/apidoc/route.php`
- তৈরি করুন: `service/app/common/Definitions.php`
- পরিবর্তন করুন: `service/config/route.php` (apidoc রাউট নিবন্ধন করুন)

- [ ] **Step 1: hg/apidoc ইনস্টল করুন** (composer.json-এ "hg/apidoc": "*" আগে থেকেই আছে, সরাসরি update করুন)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: apidoc কনফিগারেশন লিখুন** (admin টেমপ্লেট কপি করে পরিবর্তন করুন)
...
- [ ] **Step 3: Definitions.php লিখুন (সাধারণ প্রতিক্রিয়া কাঠামো)**
...
- [ ] **Step 4: apidoc রাউট নিবন্ধন করুন (config/route.php-এর শেষে যোগ করুন)**
...
- [ ] **Step 5: লোকালে জেনারেশন যাচাই করুন**
...
প্রত্যাশিত: `apidoc OK` আউটপুট (পৃষ্ঠার শিরোনাম: Social ক্লায়েন্ট API)।

- [ ] **Step 6: সব টেস্ট চালিয়ে কোনো রিগ্রেশন নেই তা নিশ্চিত করুন**
...
### Task 7 (backend-admin): ব্যবহারকারী ব্যবস্থাপনা (social_ টেবিলে সরাসরি কোয়েরি)

**ফাইল:**
- তৈরি করুন: `admin/app/model/SocialUser.php`
- তৈরি করুন: `admin/app/admin/controller/SocialUserController.php`
- পরিবর্তন করুন: `admin/config/route.php` (/admin/user রাউট গ্রুপ)

- [ ] **Step 1: SocialUser মডেল লিখুন**
...
- [ ] **Step 2: SocialUserController লিখুন**
...
- [ ] **Step 3: admin রাউট যোগ করুন (config/route.php-এর শেষে)**
...
> নোট: AuthCheck হল open-admin-এর বিদ্যমান মিডলওয়্যার (নাম আলাদা হলে admin-এর বিদ্যমান লগইন-যাচাই মিডলওয়্যার ব্যবহার করুন)। M2-এ এটি service-এ gRPC কল দিয়ে বদলানো হবে।

- [ ] **Step 4: সিনট্যাক্স যাচাই**
...
প্রত্যাশিত: `No syntax errors detected`-এর ৪টি লাইন।
...
### Task 8 (android-dev): লগইন + টাইমলাইন + পোস্ট তৈরি

**ফাইল:**
- তৈরি করুন: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- তৈরি করুন: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- তৈরি করুন: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- পরিবর্তন করুন: `apps/android/app/src/main/java/.../MainActivity.kt`
- পরিবর্তন করুন: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1: build.gradle.kts-এ ডিপেন্ডেন্সি যোগ করুন**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose বা নেটিভ View — অ্যাপের বিদ্যমান UI স্টাইল অনুযায়ী; এখানে নেটিভ View দিয়ে ন্যূনতম বাস্তবায়ন)
...
- [ ] **Step 4: কম্পাইল যাচাই**

```bash
cd apps/android && ./gradlew assembleDebug
```

প্রত্যাশিত: BUILD SUCCESSFUL.
...
### Task 9 (ios-dev): লগইন + টাইমলাইন + পোস্ট তৈরি

**ফাইল:**
- পরিবর্তন করুন: `apps/ios/.../APIClient.swift` (পুনর্লিখন)
- পরিবর্তন করুন: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> নোট: iOS প্রজেক্ট storyboard/UIKit ব্যবহার করলে ContentView-কে সমতুল্য UIKit কন্ট্রোলার দিয়ে বদলান; এখানে SwiftUI টেমপ্লেট অনুযায়ী দেওয়া হয়েছে।
...
### Task 10 (harmonyos-dev): লগইন + টাইমলাইন + পোস্ট তৈরি

**ফাইল:**
- তৈরি করুন: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- পরিবর্তন করুন: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (http মডিউল)**
...
- [ ] **Step 2: Index.ets পুনর্লিখন (loginPanel/timelinePanel বিল্ডার)**
...
- [ ] **Step 3: সিনট্যাক্স যাচাই** (DevEco পরিবেশ প্রস্তুত হলে)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

প্রত্যাশিত: BUILD SUCCESSFUL (লোকালে DevEco/hvigor না থাকলে বাদ দিন এবং CI প্রস্তুত হলে যাচাই করুন)।
...
## চূড়ান্ত যাচাই (সব কাজ শেষ হওয়ার পর)
...
কমিট করুন: `git commit -m "chore: M1 closed-loop complete"` (কোনো পরিবর্তন বাকি থাকলে)।

---

## Self-Review রেকর্ড

**1. স্পেক কভারেজ (§14-এর M1 লাইন + ব্যবহারকারী সংযোজন):**
- রেজিস্টার/লগইন/প্রোফাইল → T2, T3 ✓
- পোস্ট তৈরি/বিস্তারিত → T4 ✓
- সরলীকৃত টাইমলাইন → T4 ✓
- লাইক ও মন্তব্য → T5 ✓
- hg/apidoc ইন্টারফেস ডকুমেন্টেশন (ব্যবহারকারীর সর্বশেষ প্রয়োজন) → T6 ✓ (সব controller-এ @Apidoc অ্যানোটেশন)
- তিনটি মোবাইল ক্লায়েন্টই লগইন+টাইমলাইন+পোস্ট তৈরি বাস্তবায়ন করে → T8/T9/T10 ✓

**2. প্লেসহোল্ডার স্ক্যান:** কোনো TBD/TODO নেই; admin AuthCheck মিডলওয়্যারের নামে "আলাদা হলে বিদ্যমানটি ব্যবহার করুন" লেখা আছে, স্পষ্ট fallback রয়েছে।

**3. টাইপ সামঞ্জস্য:**
- JwtHelper::encode/decode/revoke/isRevoked T2-তে সংজ্ঞায়িত, T2 টেস্টগুলো ধারাবাহিকভাবে ব্যবহার করে
- `$request->uid`/`$request->jti` AuthMiddleware ইনজেক্ট করে, T3/T4/T5 টেস্ট সরাসরি `$req->uid` অ্যাসাইন করে ✓
- `access_token`/`refresh_token`/`expires_in` স্নেক-কেস নামকরণ T2-তে সংজ্ঞায়িত, T8/T9/T10 ক্লায়েন্ট SerialName/CodingKeys/লিটারেল দিয়ে মিলিয়ে নেয় ✓
- PostController::detail/like/unlike ও CommentController::index/create মেথডের `string $id` ইনজেকশন স্বাক্ষর একই, রাউট `{id}` মিলে যায় ✓

**4. জ্ঞাত সীমাবদ্ধতা (ইচ্ছাকৃতভাবে স্থগিত):**
- Redis না থাকলে Redis কেস বাদ পড়ে (redisAvailable())
- admin→service gRPC M2-তে স্থগিত (M1 সরাসরি টেবিল কোয়েরি করে, টেবিল প্রিফিক্স social_ যাচাইকৃত)
- ক্যাপচা/ছবি আপলোড/ফলো/সম্পূর্ণ ফিড/অনুবাদ → M2+
- created_at সেকেন্ড-স্তরের নির্ভুলতা → টাইমলাইন টেস্টে usleep(1100000) দিয়ে উল্লেখ করা আছে
