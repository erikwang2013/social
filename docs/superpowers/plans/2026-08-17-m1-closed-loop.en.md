# M1 Closed-Loop Milestone Implementation Plan
...
**Goal:** Complete the M1 closed loop: register/login/profile, create post/detail, simplified timeline, likes & comments, and generate hg/apidoc API documentation for all service APIs.

**Architecture:** service (webman) provides all client-facing HTTP APIs (`/api/v1`) with JWT dual-token auth (access 2h + refresh 14d, jti Redis blacklist for logout), Eloquent connecting directly to MySQL (table prefix `social_`); admin provides user management (M1 queries tables directly, gRPC deferred to M2); three mobile clients (Android/iOS/HarmonyOS) implement login + timeline + create post natively. All service APIs use hg/apidoc annotations to generate API docs.
...
**Known limitations (deferred):** captcha, image upload, follows, full feed, admin→service gRPC, content translation → all M2+.
...
### Task 1 (backend-service): Database layer + test skeleton

**Files:**
- Create: `service/config/database.php`
- Create: `service/database/migrations/m1.sql`
- Create: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Create: `service/phpunit.xml`
- Create: `service/tests/bootstrap.php`
- Create: `service/tests/AuthModelTest.php`
- Modify: `service/composer.json` (webman/database + phpunit dev)
...
- [ ] **Step 2: Write config/database.php** (mimic admin/config/database.php: default mysql, test connection uses sqlite :memory:)
...
- [ ] **Step 3: Write m1.sql** (structure only, for production/local mysql; tests create tables inline via bootstrap)
...
- [ ] **Step 4: Write the models** (5 models; Eloquent reads the prefix from config('database'))
...
- [ ] **Step 5: Write phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: Write tests/AuthModelTest.php**
...
### Task 2 (backend-service): Authentication (register/login/refresh/logout/me)

**Files:**
- Create: `service/app/common/JwtHelper.php`
- Create: `service/app/middleware/AuthMiddleware.php`
- Create: `service/app/controller/AuthController.php`
- Create: `service/config/route.php`
- Create: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Create: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Modify: `service/app/functions.php` (add redisAvailable())

- [ ] **Step 1: Install the JWT package (skip if already in composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: jwt.php config**
...
- [ ] **Step 3: Append redisAvailable() to app/functions.php** (so tests can skip Redis-dependent cases)
...
> Note: erikwang2013/jwt-webman's JWTFactory::createFromConfig also works; to keep the tests simple (not depending on its internals), we use its underlying firebase/php-jwt directly. If the package doesn't ship firebase/jwt, use the `erikwang2013\jwt\JWTFactory` alternative:
...
- [ ] **Step 6: AuthController** (with @Apidoc annotations — Task 6 enables apidoc generation, write them here now)
...
- [ ] **Step 7: Routes config/route.php** (Task 6 will append the apidoc register)
...
- [ ] **Step 8: Tests tests/AuthControllerTest.php**
...
- [ ] **Step 9: Tests tests/AuthFlowTest.php** (Redis-dependent cases run only when redisAvailable())
...
Expected: all PASS (Redis cases skipped or passed).
...
### Task 3 (backend-service): Profile API (me update)

**Files:**
- Create: `service/app/controller/MeController.php`
- Create: `service/tests/MeControllerTest.php`

- [ ] **Step 1: Write MeController**
...
### Task 4 (backend-service): Posts (create/timeline/detail)

**Files:**
- Create: `service/app/controller/PostController.php`
- Create: `service/tests/PostControllerTest.php`

- [ ] **Step 1: Write PostController (create/timeline/detail)**
...
- [ ] **Step 2: Write PostControllerTest** (the timeline ordering test needs usleep(1100000) because created_at has second-level precision)
...
Expected: all PASS (timeline test takes ~2-3 seconds).
...
### Task 5 (backend-service): Likes + Comments

**Files:**
- Modify: `service/app/controller/PostController.php` (append like/unlike)
- Create: `service/app/controller/CommentController.php`
- Create: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: Append like/unlike to PostController**
...
- [ ] **Step 2: Write CommentController**
...
- [ ] **Step 3: Write PostInteractionTest**
...
### Task 6 (backend-service): hg/apidoc API documentation

**Files:**
- Create: `service/config/plugin/hg/apidoc/app.php`
- Create: `service/config/plugin/hg/apidoc/route.php`
- Create: `service/app/common/Definitions.php`
- Modify: `service/config/route.php` (register apidoc routes)

- [ ] **Step 1: Install hg/apidoc** (composer.json already has "hg/apidoc": "*", just update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: Write the apidoc config** (copy the admin template and modify)
...
- [ ] **Step 3: Write Definitions.php (common response structure)**
...
- [ ] **Step 4: Register apidoc routes (append to end of config/route.php)**
...
- [ ] **Step 5: Verify generation locally**
...
Expected: outputs `apidoc OK` (page title: Social client API).
...
### Task 7 (backend-admin): User management (direct query on social_ tables)

**Files:**
- Create: `admin/app/model/SocialUser.php`
- Create: `admin/app/admin/controller/SocialUserController.php`
- Modify: `admin/config/route.php` (/admin/user route group)

- [ ] **Step 1: Write the SocialUser model**
...
- [ ] **Step 2: Write SocialUserController**
...
- [ ] **Step 3: Append admin routes (end of config/route.php)**
...
> Note: AuthCheck is an existing open-admin middleware (if the name differs, use admin's existing login-check middleware). M2 will switch this to gRPC calls to service.
...
Expected: 4 lines of `No syntax errors detected`.
...
### Task 8 (android-dev): Login + Timeline + Create post

**Files:**
- Create: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Create: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Create: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- Modify: `apps/android/app/src/main/java/.../MainActivity.kt`
- Modify: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose or native Views — follow the app's existing UI style; here we use a minimal native View implementation)
...
### Task 9 (ios-dev): Login + Timeline + Create post

**Files:**
- Modify: `apps/ios/.../APIClient.swift` (rewrite)
- Modify: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> Note: if the iOS project uses storyboard/UIKit, replace ContentView with an equivalent UIKit controller; here we follow the SwiftUI template.
...
### Task 10 (harmonyos-dev): Login + Timeline + Create post

**Files:**
- Create: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- Modify: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (http module)**
...
- [ ] **Step 2: Rewrite Index.ets (loginPanel/timelinePanel builders)**
...
- [ ] **Step 3: Syntax check** (once the DevEco environment is ready)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Expected: BUILD SUCCESSFUL (skip if DevEco/hvigor is not installed locally; verify once CI is ready).
...
Commit: `git commit -m "chore: M1 closed-loop complete"` (if any changes remain).

---

## Self-Review Log

**1. Spec coverage (§14 M1 rows + user additions):**
- Register/login/profile → T2, T3 ✓
- Create post/detail → T4 ✓
- Simplified timeline → T4 ✓
- Likes & comments → T5 ✓
- hg/apidoc API docs (latest user requirement) → T6 ✓ (all controllers carry @Apidoc annotations)
- All three mobile clients implement login+timeline+create post → T8/T9/T10 ✓

**2. Placeholder scan:** no TBD/TODO; the admin AuthCheck middleware name notes "use the existing one if different", with a clear fallback.

**3. Type consistency:**
- JwtHelper::encode/decode/revoke/isRevoked are defined in T2; T2 tests use them consistently
- `$request->uid`/`$request->jti` are injected by AuthMiddleware; T3/T4/T5 tests assign `$req->uid` directly ✓
- Response fields `access_token`/`refresh_token`/`expires_in` use snake_case as defined in T2; T8/T9/T10 clients align via SerialName/CodingKeys/literals ✓
- PostController::detail/like/unlike and CommentController::index/create inject `string $id` with matching signatures; route `{id}` maps accordingly ✓

**4. Known limitations (intentionally deferred):**
- Redis cases are skipped when Redis is unavailable (redisAvailable())
- admin→service gRPC is deferred to M2 (M1 queries tables directly; table prefix social_ verified)
- Captcha/image upload/follows/full feed/translation → M2+
- created_at second-level precision → noted in the timeline test's usleep(1100000)
