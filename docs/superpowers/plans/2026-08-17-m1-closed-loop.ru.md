# План реализации вехи M1 (замкнутый контур)
...
**Цель:** завершить замкнутый контур M1: регистрация/вход/профиль, создание поста/детали, упрощённая лента, лайки и комментарии, а также сгенерировать документацию интерфейсов hg/apidoc для всех API сервиса service.

**Архитектура:** service (webman) предоставляет все пользовательские HTTP API (`/api/v1`) с JWT-аутентификацией по двум токенам (access 2ч + refresh 14д, jti Redis blacklist для выхода), Eloquent напрямую подключается к MySQL (префикс таблиц `social_`); admin предоставляет управление пользователями (в M1 — прямой запрос к таблицам, gRPC перенесён на M2); три мобильных клиента (Android/iOS/HarmonyOS) реализуют вход + ленту + создание поста нативно. Все API сервиса используют аннотации hg/apidoc для генерации документации.

**Технологический стек:** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**Известные ограничения (отложено):** капча, загрузка изображений, подписки, полная лента, gRPC admin→service, перевод контента → всё на M2+.
...
### Task 1 (backend-service): слой базы данных + тестовый каркас

**Файлы:**
- Создать: `service/config/database.php`
- Создать: `service/database/migrations/m1.sql`
- Создать: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Создать: `service/phpunit.xml`
- Создать: `service/tests/bootstrap.php`
- Создать: `service/tests/AuthModelTest.php`
- Изменить: `service/composer.json` (webman/database + phpunit dev)
...
- [ ] **Step 2: написать config/database.php** (по образцу admin/config/database.php: default — mysql, тестовое соединение — sqlite :memory:)
...
- [ ] **Step 3: написать m1.sql** (только структура, для продакшена/локального mysql; тесты создают таблицы инлайн через bootstrap)
...
- [ ] **Step 4: написать модели** (5 моделей; Eloquent читает prefix из config('database'))
...
- [ ] **Step 5: написать phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: написать тест tests/AuthModelTest.php**
...
Ожидается: 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service): аутентификация (регистрация/вход/обновление/выход/me)

**Файлы:**
- Создать: `service/app/common/JwtHelper.php`
- Создать: `service/app/middleware/AuthMiddleware.php`
- Создать: `service/app/controller/AuthController.php`
- Создать: `service/config/route.php`
- Создать: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Создать: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Изменить: `service/app/functions.php` (добавить redisAvailable())

- [ ] **Step 1: установить пакет JWT (пропустить, если уже есть в composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: конфигурация jwt.php**
...
- [ ] **Step 3: добавить redisAvailable() в app/functions.php** (чтобы тесты могли пропускать кейсы, зависящие от Redis)
...
> Примечание: можно также использовать JWTFactory::createFromConfig из erikwang2013/jwt-webman; чтобы тесты оставались простыми (не зависели от его внутренностей), напрямую используется его нижний слой firebase/php-jwt. Если в пакете нет firebase/jwt, используйте вариант `erikwang2013\jwt\JWTFactory`:
...
- [ ] **Step 6: AuthController** (с аннотациями @Apidoc — Task 6 включит генерацию apidoc, пишем их сразу)
...
- [ ] **Step 7: маршруты config/route.php** (Task 6 добавит регистрацию apidoc)
...
- [ ] **Step 8: тест tests/AuthControllerTest.php**
...
- [ ] **Step 9: тест tests/AuthFlowTest.php** (кейсы, зависящие от Redis, выполняются только при redisAvailable())
...
Ожидается: всё PASS (Redis-кейсы пропущены или пройдены).
...
### Task 3 (backend-service): API профиля (обновление me)

**Файлы:**
- Создать: `service/app/controller/MeController.php`
- Создать: `service/tests/MeControllerTest.php`

- [ ] **Step 1: написать MeController**
...
- [ ] **Step 2: написать MeControllerTest**
...
### Task 4 (backend-service): посты (создание/лента/детали)

**Файлы:**
- Создать: `service/app/controller/PostController.php`
- Создать: `service/tests/PostControllerTest.php`

- [ ] **Step 1: написать PostController (create/timeline/detail)**
...
- [ ] **Step 2: написать PostControllerTest** (тесту сортировки ленты нужен usleep(1100000), так как created_at имеет точность до секунды)
...
Ожидается: всё PASS (тест ленты занимает ~2-3 секунды).
...
### Task 5 (backend-service): лайки + комментарии

**Файлы:**
- Изменить: `service/app/controller/PostController.php` (добавить like/unlike)
- Создать: `service/app/controller/CommentController.php`
- Создать: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: добавить like/unlike в PostController**
...
- [ ] **Step 2: написать CommentController**
...
- [ ] **Step 3: написать PostInteractionTest**
...
### Task 6 (backend-service): документация интерфейсов hg/apidoc

**Файлы:**
- Создать: `service/config/plugin/hg/apidoc/app.php`
- Создать: `service/config/plugin/hg/apidoc/route.php`
- Создать: `service/app/common/Definitions.php`
- Изменить: `service/config/route.php` (зарегистрировать маршруты apidoc)

- [ ] **Step 1: установить hg/apidoc** (в composer.json уже есть "hg/apidoc": "*", просто update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: написать конфигурацию apidoc** (скопировать шаблон из admin и изменить)
...
- [ ] **Step 3: написать Definitions.php (общая структура ответа)**
...
- [ ] **Step 4: зарегистрировать маршруты apidoc (добавить в конец config/route.php)**
...
- [ ] **Step 5: локально проверить генерацию**
...
Ожидается: вывод `apidoc OK` (заголовок страницы: клиентский API Social).

- [ ] **Step 6: запустить все тесты и убедиться в отсутствии регрессий**
...
### Task 7 (backend-admin): управление пользователями (прямой запрос к таблицам social_)

**Файлы:**
- Создать: `admin/app/model/SocialUser.php`
- Создать: `admin/app/admin/controller/SocialUserController.php`
- Изменить: `admin/config/route.php` (группа маршрутов /admin/user)

- [ ] **Step 1: написать модель SocialUser**
...
- [ ] **Step 2: написать SocialUserController**
...
- [ ] **Step 3: добавить admin-маршруты (в конец config/route.php)**
...
> Примечание: AuthCheck — существующий middleware open-admin (если имя другое, используйте существующий middleware проверки входа из admin). В M2 будет заменён на gRPC-вызовы service.
...
Ожидается: 4 строки `No syntax errors detected`.
...
### Task 8 (android-dev): вход + лента + создание поста

**Файлы:**
- Создать: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Создать: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Создать: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- Изменить: `apps/android/app/src/main/java/.../MainActivity.kt`
- Изменить: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1: добавить зависимости в build.gradle.kts**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose или нативные View — по существующему UI-стилю приложения; здесь минимальная реализация на нативных View)
...
### Task 9 (ios-dev): вход + лента + создание поста

**Файлы:**
- Изменить: `apps/ios/.../APIClient.swift` (переписать)
- Изменить: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> Примечание: если проект iOS использует storyboard/UIKit, замените ContentView эквивалентным UIKit-контроллером; здесь приведён вариант по шаблону SwiftUI.
...
### Task 10 (harmonyos-dev): вход + лента + создание поста

**Файлы:**
- Создать: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- Изменить: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (модуль http)**
...
- [ ] **Step 2: переписать Index.ets (билдеры loginPanel/timelinePanel)**
...
- [ ] **Step 3: проверка синтаксиса** (после готовности среды DevEco)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Ожидается: BUILD SUCCESSFUL (если локально нет DevEco/hvigor, пропустить и проверить после готовности CI).
...
## Финальная проверка (после выполнения всех задач)
...
Закоммитить: `git commit -m "chore: M1 closed-loop complete"` (если остались изменения).

---

## Журнал Self-Review

**1. Покрытие спеки (строки §14 для M1 + дополнения пользователя):**
- Регистрация/вход/профиль → T2, T3 ✓
- Создание поста/детали → T4 ✓
- Упрощённая лента → T4 ✓
- Лайки и комментарии → T5 ✓
- Документация интерфейсов hg/apidoc (последнее требование пользователя) → T6 ✓ (все контроллеры с аннотациями @Apidoc)
- Все три мобильных клиента реализуют вход+ленту+создание поста → T8/T9/T10 ✓

**2. Скан плейсхолдеров:** TBD/TODO нет; имя middleware AuthCheck в admin помечено "если другое — использовать существующий", есть явный fallback.

**3. Согласованность типов:**
- JwtHelper::encode/decode/revoke/isRevoked определены в T2, тесты T2 используют их согласованно
- `$request->uid`/`$request->jti` внедряются AuthMiddleware, тесты T3/T4/T5 присваивают `$req->uid` напрямую ✓
- Поля ответа `access_token`/`refresh_token`/`expires_in` в snake_case определены в T2, клиенты T8/T9/T10 выравнивают через SerialName/CodingKeys/литералы ✓
- PostController::detail/like/unlike и CommentController::index/create имеют одинаковую сигнатуру внедрения `string $id`, маршрут `{id}` соответствует ✓

**4. Известные ограничения (намеренно отложено):**
- Redis-кейсы пропускаются при отсутствии Redis (redisAvailable())
- gRPC admin→service отложен до M2 (в M1 прямой запрос к таблицам, префикс social_ проверен)
- Капча/загрузка изображений/подписки/полная лента/перевод → M2+
- created_at с точностью до секунды → отмечено usleep(1100000) в тесте ленты
