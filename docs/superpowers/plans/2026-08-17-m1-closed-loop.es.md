# Plan de implementación del hito M1 de bucle cerrado
...
**Objetivo:** completar el bucle cerrado de M1: registro/inicio de sesión/perfil, publicación de post/detalle, línea de tiempo simplificada, likes y comentarios, y generar la documentación de interfaz hg/apidoc para todas las API de service.

**Arquitectura:** service (webman) proporciona todas las API HTTP de cliente (`/api/v1`) con autenticación JWT de doble token (access 2 h + refresh 14 d, jti Redis blacklist para cerrar sesión), Eloquent conectado directamente a MySQL (prefijo de tabla `social_`); admin ofrece la gestión de usuarios (en M1 consulta directa a tablas, gRPC diferido a M2); los tres clientes móviles (Android/iOS/HarmonyOS) implementan de forma nativa inicio de sesión + línea de tiempo + publicar post. Todas las API de service usan anotaciones hg/apidoc para generar la documentación.

**Stack tecnológico:** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**Limitaciones conocidas (diferidas):** captcha, subida de imágenes, seguimientos, feed completo, gRPC admin→service, traducción de contenido → todo en M2+.
...
### Task 1 (backend-service): capa de base de datos + esqueleto de pruebas

**Archivos:**
- Crear: `service/config/database.php`
- Crear: `service/database/migrations/m1.sql`
- Crear: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Crear: `service/phpunit.xml`
- Crear: `service/tests/bootstrap.php`
- Crear: `service/tests/AuthModelTest.php`
- Modificar: `service/composer.json` (webman/database + phpunit dev)
...
- [ ] **Step 2: escribir config/database.php** (imitando admin/config/database.php: default mysql, conexión de prueba con sqlite :memory:)
...
- [ ] **Step 3: escribir m1.sql** (solo estructura, para mysql de producción/local; las pruebas crean las tablas en línea mediante bootstrap)
...
- [ ] **Step 4: escribir los modelos** (5 modelos; Eloquent lee el prefijo de config('database'))
...
- [ ] **Step 5: escribir phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: escribir la prueba tests/AuthModelTest.php**
...
Esperado: 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service): autenticación (registro/inicio de sesión/refresco/cierre de sesión/me)

**Archivos:**
- Crear: `service/app/common/JwtHelper.php`
- Crear: `service/app/middleware/AuthMiddleware.php`
- Crear: `service/app/controller/AuthController.php`
- Crear: `service/config/route.php`
- Crear: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Crear: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Modificar: `service/app/functions.php` (añadir redisAvailable())

- [ ] **Step 1: instalar el paquete JWT (omitir si ya está en composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: configuración de jwt.php**
...
- [ ] **Step 3: añadir redisAvailable() a app/functions.php** (para que las pruebas omitan los casos que dependen de Redis)
...
> Nota: JWTFactory::createFromConfig de erikwang2013/jwt-webman también sirve; para mantener las pruebas simples (sin depender de sus internos), se usa directamente su capa subyacente firebase/php-jwt. Si el paquete no trae firebase/jwt, usar la variante `erikwang2013\jwt\JWTFactory`:
...
- [ ] **Step 6: AuthController** (con anotaciones @Apidoc — Task 6 habilitará la generación de apidoc, escribirlas aquí directamente)
...
- [ ] **Step 7: rutas config/route.php** (Task 6 añadirá el registro de apidoc)
...
- [ ] **Step 8: prueba tests/AuthControllerTest.php**
...
- [ ] **Step 9: prueba tests/AuthFlowTest.php** (los casos que dependen de Redis solo se ejecutan cuando redisAvailable())
...
- [ ] **Step 10: Ejecutar las pruebas**

```bash
cd service && vendor/bin/phpunit
```

Esperado: todo PASS (casos Redis omitidos o superados).
...
### Task 3 (backend-service): API de perfil (actualización de me)

**Archivos:**
- Crear: `service/app/controller/MeController.php`
- Crear: `service/tests/MeControllerTest.php`

- [ ] **Step 1: escribir MeController**
...
- [ ] **Step 2: escribir MeControllerTest**
...
### Task 4 (backend-service): posts (publicación/línea de tiempo/detalle)

**Archivos:**
- Crear: `service/app/controller/PostController.php`
- Crear: `service/tests/PostControllerTest.php`

- [ ] **Step 1: escribir PostController (create/timeline/detail)**
...
- [ ] **Step 2: escribir PostControllerTest** (la prueba de orden de la línea de tiempo necesita usleep(1100000) porque created_at tiene precisión de segundos)
...
Esperado: todo PASS (la prueba de la línea de tiempo tarda unos 2-3 s).
...
### Task 5 (backend-service): likes + comentarios

**Archivos:**
- Modificar: `service/app/controller/PostController.php` (añadir like/unlike)
- Crear: `service/app/controller/CommentController.php`
- Crear: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: añadir like/unlike a PostController**
...
- [ ] **Step 2: escribir CommentController**
...
- [ ] **Step 3: escribir PostInteractionTest**
...
### Task 6 (backend-service): documentación de interfaz hg/apidoc

**Archivos:**
- Crear: `service/config/plugin/hg/apidoc/app.php`
- Crear: `service/config/plugin/hg/apidoc/route.php`
- Crear: `service/app/common/Definitions.php`
- Modificar: `service/config/route.php` (registrar las rutas de apidoc)

- [ ] **Step 1: instalar hg/apidoc** (composer.json ya tiene "hg/apidoc": "*", solo hacer update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: escribir la configuración de apidoc** (copiar la plantilla de admin y modificar)
...
- [ ] **Step 3: escribir Definitions.php (estructura de respuesta común)**
...
- [ ] **Step 4: registrar las rutas de apidoc (añadir al final de config/route.php)**
...
- [ ] **Step 5: verificar la generación en local**
...
Esperado: salida `apidoc OK` (título de página: API de cliente Social).

- [ ] **Step 6: ejecutar todas las pruebas para confirmar que no hay regresiones**
...
### Task 7 (backend-admin): gestión de usuarios (consulta directa a las tablas social_)

**Archivos:**
- Crear: `admin/app/model/SocialUser.php`
- Crear: `admin/app/admin/controller/SocialUserController.php`
- Modificar: `admin/config/route.php` (grupo de rutas /admin/user)

- [ ] **Step 1: escribir el modelo SocialUser**
...
- [ ] **Step 2: escribir SocialUserController**
...
- [ ] **Step 3: añadir las rutas de admin (final de config/route.php)**
...
> Nota: AuthCheck es un middleware existente de open-admin (si el nombre difiere, usar el middleware de verificación de inicio de sesión existente de admin). En M2 se cambiará a llamadas gRPC a service.
...
Esperado: 4 líneas de `No syntax errors detected`.
...
### Task 8 (android-dev): inicio de sesión + línea de tiempo + publicar post

**Archivos:**
- Crear: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Crear: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Crear: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- Modificar: `apps/android/app/src/main/java/.../MainActivity.kt`
- Modificar: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1: añadir dependencias a build.gradle.kts**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose o View nativas — siguiendo el estilo de UI existente de la app; aquí una implementación mínima con View nativas)
...
### Task 9 (ios-dev): inicio de sesión + línea de tiempo + publicar post

**Archivos:**
- Modificar: `apps/ios/.../APIClient.swift` (reescribir)
- Modificar: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> Nota: si el proyecto iOS usa storyboard/UIKit, reemplazar ContentView por un controlador UIKit equivalente; aquí se muestra según la plantilla SwiftUI.
...
### Task 10 (harmonyos-dev): inicio de sesión + línea de tiempo + publicar post

**Archivos:**
- Crear: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- Modificar: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (módulo http)**
...
- [ ] **Step 2: reescribir Index.ets (builders loginPanel/timelinePanel)**
...
- [ ] **Step 3: verificación de sintaxis** (cuando el entorno DevEco esté listo)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Esperado: BUILD SUCCESSFUL (si no hay DevEco/hvigor en local, omitir y verificar cuando la CI esté lista).
...
Confirmar: `git commit -m "chore: M1 closed-loop complete"` (si quedan cambios).

---

## Registro de Self-Review

**1. Cobertura del spec (filas §14 de M1 + adiciones del usuario):**
- Registro/inicio de sesión/perfil → T2, T3 ✓
- Publicación de post/detalle → T4 ✓
- Línea de tiempo simplificada → T4 ✓
- Likes y comentarios → T5 ✓
- Documentación de interfaz hg/apidoc (último requisito del usuario) → T6 ✓ (todos los controladores llevan anotaciones @Apidoc)
- Los tres clientes móviles implementan inicio de sesión+línea de tiempo+publicar post → T8/T9/T10 ✓

**2. Escaneo de placeholders:** sin TBD/TODO; el nombre del middleware AuthCheck de admin indica "si es diferente, usar el existente", con un fallback claro.

**3. Consistencia de tipos:**
- JwtHelper::encode/decode/revoke/isRevoked se definen en T2, y las pruebas de T2 los usan de forma coherente
- `$request->uid`/`$request->jti` los inyecta AuthMiddleware; las pruebas T3/T4/T5 asignan `$req->uid` directamente ✓
- Los campos de respuesta `access_token`/`refresh_token`/`expires_in` con snake_case se definen en T2; los clientes T8/T9/T10 se alinean con SerialName/CodingKeys/literales ✓
- PostController::detail/like/unlike y CommentController::index/create inyectan `string $id` con firmas idénticas; la ruta `{id}` se corresponde ✓

**4. Limitaciones conocidas (diferidas a propósito):**
- Los casos Redis se omiten cuando no hay Redis (redisAvailable())
- gRPC admin→service diferido a M2 (M1 consulta las tablas directamente; prefijo social_ verificado)
- Captcha/subida de imágenes/seguimientos/feed completo/traducción → M2+
- Precisión de segundos de created_at → anotado en la prueba de la línea de tiempo con usleep(1100000)
