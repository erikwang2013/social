...
**Goal:** 完成 M1 闭环：注册/登录/资料、发动态/详情、简化时间线、点赞评论，并为 service 全部 API 生成 hg/apidoc 接口文档。

**Architecture:** service（webman）提供全部用户端 HTTP API（`/api/v1`），JWT 双 token 认证（access 2h + refresh 14d，jti Redis blacklist 登出），Eloquent 直连 MySQL（表前缀 `social_`）；admin 端提供用户管理（M1 直接查表，gRPC 化延迟到 M2）；三个移动端（Android/iOS/HarmonyOS）原生实现登录 + 时间线 + 发动态。所有 service API 使用 hg/apidoc 注解生成接口文档。
...
**已知限制（延迟项）：** 验证码、图片上传、关注、完整信息流、admin→service gRPC 化、内容翻译 → 全部 M2+。
...
### Task 1 (backend-service): 数据库层 + 测试骨架
...
- [ ] **Step 2: 写 config/database.php**（模仿 admin/config/database.php，default mysql，test 连接用 sqlite :memory:）
...
- [ ] **Step 3: 写 m1.sql**（仅结构，供生产/本地 mysql 执行；测试用 bootstrap 内联建表）
...
- [ ] **Step 4: 写模型**（5 个模型，Eloquent 从 config('database') 读 prefix）
...
- [ ] **Step 5: 写 phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: 写测试 tests/AuthModelTest.php**
...
### Task 2 (backend-service): 认证（注册/登录/刷新/登出/me）
...
- Modify: `service/app/functions.php`（加 redisAvailable()）

- [ ] **Step 1: 安装 JWT 包（若 composer.json 已有则跳过）**
...
- [ ] **Step 3: app/functions.php 追加 redisAvailable()**（供测试跳过 Redis 相关用例）
...
> 注：erikwang2013/jwt-webman 的 JWTFactory::createFromConfig 也可用；为保持测试简单（不依赖其内部），直接使用其底层 firebase/php-jwt。若包未带 firebase/jwt，改用 `erikwang2013\jwt\JWTFactory` 方案：
...
- [ ] **Step 6: AuthController**（含 @Apidoc 注解——Task 6 会启用 apidoc 生成，此处直接写上）
...
- [ ] **Step 7: 路由 config/route.php**（Task 6 会追加 apidoc register）
...
- [ ] **Step 8: 测试 tests/AuthControllerTest.php**
...
- [ ] **Step 9: 测试 tests/AuthFlowTest.php**（Redis 依赖用例仅在 redisAvailable() 时执行）
...
Expected: 全部 PASS（Redis 用例跳过或通过）。
...
- [ ] **Step 2: 写 PostControllerTest**（timeline 排序测试需 usleep(1100000) 区分 created_at 秒级精度）
...
Expected: 全部 PASS（timeline 测试约 2-3 秒）。
...
### Task 5 (backend-service): 点赞 + 评论

**Files:**
- Modify: `service/app/controller/PostController.php`（追加 like/unlike）
- Create: `service/app/controller/CommentController.php`
- Create: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: PostController 追加 like/unlike**
...
### Task 6 (backend-service): hg/apidoc 接口文档
...
- Modify: `service/config/route.php`（注册 apidoc 路由）

- [ ] **Step 1: 安装 hg/apidoc**（composer.json 已有 "hg/apidoc": "*"，直接 update）

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: 写 apidoc 配置**（复制 admin 模板修改）
...
- [ ] **Step 3: 写 Definitions.php（公共返回结构）**
...
- [ ] **Step 4: 注册 apidoc 路由（追加到 config/route.php 末尾）**
...
Expected: 输出 `apidoc OK`（页面标题 Social 用户端 API）。
...
### Task 7 (backend-admin): 用户管理（直查 social_ 表）

**Files:**
- Create: `admin/app/model/SocialUser.php`
- Create: `admin/app/admin/controller/SocialUserController.php`
- Modify: `admin/config/route.php`（/admin/user 路由组）

- [ ] **Step 1: 写 SocialUser 模型**
...
- [ ] **Step 3: 追加 admin 路由（config/route.php 末尾）**
...
> 注：AuthCheck 是 open-admin 已有中间件（若名称不同，用 admin 现有的登录校验中间件）。M2 将改为 gRPC 调用 service。
...
Expected: 4 行 `No syntax errors detected`。
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt**（Compose 或原生 View——按 app 现有 UI 风格；这里用原生 View 最小实现）
...
### Task 9 (ios-dev): 登录 + 时间线 + 发动态

**Files:**
- Modify: `apps/ios/.../APIClient.swift`（重写）
...
> 注：iOS 项目若用 storyboard/UIKit，将 ContentView 替换为等价 UIKit 控制器；此处按 SwiftUI 模板给出。
...
### Task 10 (harmonyos-dev): 登录 + 时间线 + 发动态
...
- [ ] **Step 2: Index.ets 重写（loginPanel/timelinePanel builders）**
...
提交：`git commit -m "chore: M1 closed-loop complete"`（如有剩余改动）。

---

## Self-Review 记录

**1. Spec coverage（§14 M1 行 + 用户补充）：**
- 注册/登录/资料 → T2、T3 ✓
- 发动态/详情 → T4 ✓
- 简化时间线 → T4 ✓
- 点赞评论 → T5 ✓
- hg/apidoc 接口文档（用户最新要求）→ T6 ✓（所有 controller 带 @Apidoc 注解）
- 三个移动端各实现登录+时间线+发动态 → T8/T9/T10 ✓

**2. Placeholder scan：** 无 TBD/TODO；admin AuthCheck 中间件名称标注了"若不同用现有的"，有明确 fallback。

**3. Type consistency：**
- JwtHelper::encode/decode/revoke/isRevoked 在 T2 定义，T2 测试使用一致
- `$request->uid`/`$request->jti` 由 AuthMiddleware 注入，T3/T4/T5 测试直接赋值 `$req->uid` ✓
- 响应字段 `access_token`/`refresh_token`/`expires_in` 下划线命名在 T2 定义，T8/T9/T10 客户端用 SerialName/CodingKeys/字面量对齐 ✓
- PostController::detail/like/unlike 与 CommentController::index/create 方法注入 `string $id` 签名一致，路由 `{id}` 对应 ✓

**4. 已知限制（有意延迟）：**
- Redis 用例在无 Redis 时跳过（redisAvailable()）
- admin→service gRPC 化延迟到 M2（M1 直查表，已验证表前缀 social_）
- 验证码/图片上传/关注/完整信息流/翻译 → M2+
- created_at 秒级精度 → timeline 测试 usleep(1100000) 已标注
