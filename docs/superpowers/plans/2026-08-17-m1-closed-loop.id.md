# Rencana Implementasi M1 Closed-Loop (Tonggak)
...
**Goal:** Selesaikan closed-loop M1: registrasi/login/profil, buat postingan/detail, timeline sederhana, suka & komentar, serta buat dokumentasi API hg/apidoc untuk semua API service.

**Arsitektur:** service (webman) menyediakan semua API HTTP sisi klien (`/api/v1`) dengan autentikasi JWT dual-token (access 2h + refresh 14d, blacklist jti Redis untuk logout), Eloquent terhubung langsung ke MySQL (prefiks tabel `social_`); admin menyediakan manajemen pengguna (M1 query tabel langsung, gRPC ditunda ke M2); tiga klien seluler (Android/iOS/HarmonyOS) mengimplementasikan login + timeline + buat postingan secara native. Semua API service menggunakan anotasi hg/apidoc untuk membuat dokumentasi API.
...
**Keterbatasan yang diketahui (ditunda):** captcha, unggah gambar, follow, feed lengkap, gRPC admin→service, terjemahan konten → semua M2+.
...
### Task 1 (backend-service): Lapisan database + kerangka tes

**Berkas:**
- Buat: `service/config/database.php`
- Buat: `service/database/migrations/m1.sql`
- Buat: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Buat: `service/phpunit.xml`
- Buat: `service/tests/bootstrap.php`
- Buat: `service/tests/AuthModelTest.php`
- Ubah: `service/composer.json` (webman/database + phpunit dev)
...
- [ ] **Step 2: Tulis config/database.php** (tiru admin/config/database.php: default mysql, koneksi test memakai sqlite :memory:)
...
- [ ] **Step 3: Tulis m1.sql** (hanya struktur, untuk mysql produksi/lokal; tes membuat tabel inline lewat bootstrap)
...
- [ ] **Step 4: Tulis model** (5 model; Eloquent membaca prefix dari config('database'))
...
- [ ] **Step 5: Tulis phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: Tulis tes tests/AuthModelTest.php**
...
Diharapkan: 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service): Autentikasi (register/login/refresh/logout/me)

**Berkas:**
- Buat: `service/app/common/JwtHelper.php`
- Buat: `service/app/middleware/AuthMiddleware.php`
- Buat: `service/app/controller/AuthController.php`
- Buat: `service/config/route.php`
- Buat: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Buat: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Ubah: `service/app/functions.php` (tambah redisAvailable())

- [ ] **Step 1: Pasang paket JWT (lewati jika sudah ada di composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: Konfigurasi jwt.php**
...
- [ ] **Step 3: Tambahkan redisAvailable() ke app/functions.php** (agar tes bisa melewati kasus yang bergantung Redis)
...
> Catatan: JWTFactory::createFromConfig dari erikwang2013/jwt-webman juga bisa dipakai; agar tes tetap sederhana (tidak bergantung pada internalnya), kita pakai firebase/php-jwt yang menjadi basisnya. Jika paket tidak menyertakan firebase/jwt, gunakan alternatif `erikwang2013\jwt\JWTFactory`:
...
- [ ] **Step 6: AuthController** (dengan anotasi @Apidoc — Task 6 akan mengaktifkan pembuatan apidoc, tulis sekarang)
...
- [ ] **Step 7: Rute config/route.php** (Task 6 akan menambahkan register apidoc)
...
- [ ] **Step 8: Tes tests/AuthControllerTest.php**
...
- [ ] **Step 9: Tes tests/AuthFlowTest.php** (kasus bergantung Redis hanya berjalan saat redisAvailable())
...
Diharapkan: semua PASS (kasus Redis dilewati atau lulus).
...
### Task 3 (backend-service): API profil (update me)

**Berkas:**
- Buat: `service/app/controller/MeController.php`
- Buat: `service/tests/MeControllerTest.php`

- [ ] **Step 1: Tulis MeController**
...
### Task 4 (backend-service): Postingan (buat/timeline/detail)

**Berkas:**
- Buat: `service/app/controller/PostController.php`
- Buat: `service/tests/PostControllerTest.php`

- [ ] **Step 1: Tulis PostController (create/timeline/detail)**
...
- [ ] **Step 2: Tulis PostControllerTest** (tes urutan timeline perlu usleep(1100000) karena created_at berpresisi detik)
...
Diharapkan: semua PASS (tes timeline sekitar 2-3 detik).
...
### Task 5 (backend-service): Suka + Komentar

**Berkas:**
- Ubah: `service/app/controller/PostController.php` (tambahkan like/unlike)
- Buat: `service/app/controller/CommentController.php`
- Buat: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: Tambahkan like/unlike ke PostController**
...
- [ ] **Step 2: Tulis CommentController**
...
- [ ] **Step 3: Tulis PostInteractionTest**
...
### Task 6 (backend-service): Dokumentasi API hg/apidoc

**Berkas:**
- Buat: `service/config/plugin/hg/apidoc/app.php`
- Buat: `service/config/plugin/hg/apidoc/route.php`
- Buat: `service/app/common/Definitions.php`
- Ubah: `service/config/route.php` (daftarkan rute apidoc)

- [ ] **Step 1: Pasang hg/apidoc** (composer.json sudah punya "hg/apidoc": "*", tinggal update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: Tulis konfigurasi apidoc** (salin template admin lalu ubah)
...
- [ ] **Step 3: Tulis Definitions.php (struktur respons umum)**
...
- [ ] **Step 4: Daftarkan rute apidoc (tambahkan di akhir config/route.php)**
...
- [ ] **Step 5: Verifikasi pembuatan secara lokal**
...
Diharapkan: output `apidoc OK` (judul halaman: API klien Social).

- [ ] **Step 6: Jalankan semua tes untuk memastikan tidak ada regresi**
...
### Task 7 (backend-admin): Manajemen pengguna (query langsung tabel social_)

**Berkas:**
- Buat: `admin/app/model/SocialUser.php`
- Buat: `admin/app/admin/controller/SocialUserController.php`
- Ubah: `admin/config/route.php` (grup rute /admin/user)

- [ ] **Step 1: Tulis model SocialUser**
...
- [ ] **Step 2: Tulis SocialUserController**
...
- [ ] **Step 3: Tambahkan rute admin (akhir config/route.php)**
...
> Catatan: AuthCheck adalah middleware yang sudah ada di open-admin (jika namanya berbeda, pakai middleware cek login admin yang ada). M2 akan mengubahnya menjadi panggilan gRPC ke service.
...
Diharapkan: 4 baris `No syntax errors detected`.
...
### Task 8 (android-dev): Login + Timeline + Buat postingan

**Berkas:**
- Buat: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Buat: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Buat: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- Ubah: `apps/android/app/src/main/java/.../MainActivity.kt`
- Ubah: `apps/android/app/build.gradle.kts` (okhttp + kotlinx-serialization)

- [ ] **Step 1: Tambahkan dependensi ke build.gradle.kts**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose atau View native — ikuti gaya UI aplikasi yang ada; di sini pakai implementasi minimal View native)
...
### Task 9 (ios-dev): Login + Timeline + Buat postingan

**Berkas:**
- Ubah: `apps/ios/.../APIClient.swift` (tulis ulang)
- Ubah: `apps/ios/.../ContentView.swift` (LoginView + TimelineView)
...
> Catatan: jika proyek iOS memakai storyboard/UIKit, ganti ContentView dengan controller UIKit yang setara; di sini mengikuti template SwiftUI.
...
### Task 10 (harmonyos-dev): Login + Timeline + Buat postingan

**Berkas:**
- Buat: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- Ubah: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`

- [ ] **Step 1: ApiService.ets (modul http)**
...
- [ ] **Step 2: Tulis ulang Index.ets (builder loginPanel/timelinePanel)**
...
- [ ] **Step 3: Cek sintaks** (setelah lingkungan DevEco siap)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Diharapkan: BUILD SUCCESSFUL (lewati jika DevEco/hvigor tidak terpasang di mesin ini; verifikasi setelah CI siap).
...
## Verifikasi akhir (setelah semua tugas selesai)
...
Commit: `git commit -m "chore: M1 closed-loop complete"` (jika masih ada perubahan).

---

## Catatan Self-Review

**1. Cakupan spec (§14 baris M1 + tambahan pengguna):**
- Registrasi/login/profil → T2, T3 ✓
- Buat postingan/detail → T4 ✓
- Timeline sederhana → T4 ✓
- Suka & komentar → T5 ✓
- Dokumentasi API hg/apidoc (permintaan terbaru pengguna) → T6 ✓ (semua controller memakai anotasi @Apidoc)
- Ketiga klien seluler mengimplementasikan login+timeline+buat postingan → T8/T9/T10 ✓

**2. Pemindaian placeholder:** tidak ada TBD/TODO; nama middleware admin AuthCheck mencatat "pakai yang sudah ada jika berbeda", ada fallback yang jelas.

**3. Konsistensi tipe:**
- JwtHelper::encode/decode/revoke/isRevoked didefinisikan di T2; tes T2 memakainya secara konsisten
- `$request->uid`/`$request->jti` diinjeksi oleh AuthMiddleware; tes T3/T4/T5 langsung menetapkan `$req->uid` ✓
- Field respons `access_token`/`refresh_token`/`expires_in` memakai snake_case sebagaimana didefinisikan di T2; klien T8/T9/T10 menyesuaikan lewat SerialName/CodingKeys/literal ✓
- PostController::detail/like/unlike dan CommentController::index/create menginjeksi `string $id` dengan tanda tangan yang cocok; rute `{id}` dipetakan sesuai ✓

**4. Keterbatasan yang diketahui (sengaja ditunda):**
- Kasus Redis dilewati saat Redis tidak tersedia (redisAvailable())
- gRPC admin→service ditunda ke M2 (M1 query tabel langsung; prefiks tabel social_ sudah diverifikasi)
- Captcha/unggah gambar/follow/feed lengkap/terjemahan → M2+
- Presisi created_at per detik → dicatat pada usleep(1100000) di tes timeline
