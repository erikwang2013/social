...
**Goal:** M1 クローズドループを完了する: 登録/ログイン/プロフィール、投稿作成/詳細、簡易タイムライン、いいね・コメント、さらに service の全 API の hg/apidoc インターフェースドキュメントを生成する。

**アーキテクチャ:** service（webman）が全クライアント向け HTTP API（`/api/v1`）を提供し、JWT デュアルトークン認証（access 2h + refresh 14d、ログアウトは jti を Redis ブラックリスト化）、Eloquent が MySQL に直接接続（テーブルプレフィックス `social_`）; admin 側はユーザー管理を提供（M1 はテーブルを直接参照、gRPC 化は M2 へ延期）; 3 つのモバイル端末（Android/iOS/HarmonyOS）はネイティブでログイン + タイムライン + 投稿作成を実装。全 service API は hg/apidoc アノテーションでインターフェースドキュメントを生成する。
...
**既知の制限（先送り項目）:** キャプチャ、画像アップロード、フォロー、完全なフィード、admin→service の gRPC 化、コンテンツ翻訳 → すべて M2+。
...
### Task 1 (backend-service): データベース層 + テスト骨組み

**ファイル:**
- 作成: `service/config/database.php`
- 作成: `service/database/migrations/m1.sql`
- 作成: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- 作成: `service/phpunit.xml`
- 作成: `service/tests/bootstrap.php`
- 作成: `service/tests/AuthModelTest.php`
- 変更: `service/composer.json`（webman/database + phpunit dev）
...
- [ ] **Step 2: config/database.php を作成**（admin/config/database.php を模倣: default は mysql、test 接続は sqlite :memory: を使用）
...
- [ ] **Step 3: m1.sql を作成**（構造のみ。本番/ローカル mysql 用。テストは bootstrap でインラインテーブル作成）
...
- [ ] **Step 4: モデルを作成**（5 つのモデル。Eloquent は config('database') からプレフィックスを読む）
...
- [ ] **Step 5: phpunit.xml + tests/bootstrap.php を作成**
...
- [ ] **Step 6: テスト tests/AuthModelTest.php を作成**
...
期待結果: 3 tests, 3 assertions, PASS.
...
### Task 2 (backend-service): 認証（登録/ログイン/リフレッシュ/ログアウト/me）

**ファイル:**
- 作成: `service/app/common/JwtHelper.php`
- 作成: `service/app/middleware/AuthMiddleware.php`
- 作成: `service/app/controller/AuthController.php`
- 作成: `service/config/route.php`
- 作成: `service/config/plugin/erikwang2013/jwt/jwt.php`
- 作成: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- 変更: `service/app/functions.php`（redisAvailable() を追加）

- [ ] **Step 1: JWT パッケージをインストール（composer.json に既にあればスキップ）**
...
- [ ] **Step 3: app/functions.php に redisAvailable() を追加**（テストが Redis 依存のケースをスキップできるように）
...
> 注: erikwang2013/jwt-webman の JWTFactory::createFromConfig も使用可能。テストをシンプルに保つため（内部実装に依存しない）、その基盤である firebase/php-jwt を直接使う。パッケージに firebase/jwt が含まれない場合は `erikwang2013\jwt\JWTFactory` 方式に切り替える:
...
- [ ] **Step 6: AuthController**（@Apidoc アノテーション付き — Task 6 で apidoc 生成が有効になるため、今のうちに記述）
...
- [ ] **Step 7: ルート config/route.php**（Task 6 で apidoc register を追加予定）
...
- [ ] **Step 8: テスト tests/AuthControllerTest.php**
...
- [ ] **Step 9: テスト tests/AuthFlowTest.php**（Redis 依存のケースは redisAvailable() のときのみ実行）
...
期待結果: すべて PASS（Redis ケースはスキップまたは成功）。
...
### Task 3 (backend-service): プロフィール API（me 更新）

**ファイル:**
- 作成: `service/app/controller/MeController.php`
- 作成: `service/tests/MeControllerTest.php`

- [ ] **Step 1: MeController を作成**
...
### Task 4 (backend-service): 投稿（作成/タイムライン/詳細）

**ファイル:**
- 作成: `service/app/controller/PostController.php`
- 作成: `service/tests/PostControllerTest.php`

- [ ] **Step 1: PostController を作成（create/timeline/detail）**
...
- [ ] **Step 2: PostControllerTest を作成**（タイムライン順序テストは created_at の秒精度を区別するため usleep(1100000) が必要）
...
期待結果: すべて PASS（タイムラインテストは約 2〜3 秒）。
...
### Task 5 (backend-service): いいね + コメント

**ファイル:**
- 変更: `service/app/controller/PostController.php`（like/unlike を追加）
- 作成: `service/app/controller/CommentController.php`
- 作成: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: PostController に like/unlike を追加**
...
- [ ] **Step 3: PostInteractionTest を作成**
...
### Task 6 (backend-service): hg/apidoc インターフェースドキュメント

**ファイル:**
- 作成: `service/config/plugin/hg/apidoc/app.php`
- 作成: `service/config/plugin/hg/apidoc/route.php`
- 作成: `service/app/common/Definitions.php`
- 変更: `service/config/route.php`（apidoc ルートを登録）

- [ ] **Step 1: hg/apidoc をインストール**（composer.json に "hg/apidoc": "*" が既にあるので update するだけ）

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: apidoc 設定を作成**（admin テンプレートをコピーして修正）
...
- [ ] **Step 3: Definitions.php を作成（共通レスポンス構造）**
...
- [ ] **Step 4: apidoc ルートを登録（config/route.php の末尾に追加）**
...
期待結果: `apidoc OK` と出力（ページタイトル: Social クライアント API）。
...
### Task 7 (backend-admin): ユーザー管理（social_ テーブルを直接参照）

**ファイル:**
- 作成: `admin/app/model/SocialUser.php`
- 作成: `admin/app/admin/controller/SocialUserController.php`
- 変更: `admin/config/route.php`（/admin/user ルートグループ）

- [ ] **Step 1: SocialUser モデルを作成**
...
- [ ] **Step 2: SocialUserController を作成**
...
- [ ] **Step 3: admin ルートを追加（config/route.php の末尾）**
...
> 注: AuthCheck は open-admin に既存のミドルウェア（名前が異なる場合は admin の既存ログイン検証ミドルウェアを使用）。M2 では gRPC で service を呼び出す形に変更。
...
期待結果: `No syntax errors detected` が 4 行。
...
### Task 8 (android-dev): ログイン + タイムライン + 投稿作成

**ファイル:**
- 作成: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- 作成: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- 作成: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- 変更: `apps/android/app/src/main/java/.../MainActivity.kt`
- 変更: `apps/android/app/build.gradle.kts`（okhttp + kotlinx-serialization）

- [ ] **Step 1: build.gradle.kts に依存関係を追加**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt**（Compose またはネイティブ View — アプリの既存 UI スタイルに従う。ここではネイティブ View の最小実装を使用）
...
### Task 9 (ios-dev): ログイン + タイムライン + 投稿作成

**ファイル:**
- 変更: `apps/ios/.../APIClient.swift`（書き直し）
- 変更: `apps/ios/.../ContentView.swift`（LoginView + TimelineView）
...
> 注: iOS プロジェクトが storyboard/UIKit を使用する場合は ContentView を同等の UIKit コントローラに置き換える。ここでは SwiftUI テンプレートに従って示す。
...
### Task 10 (harmonyos-dev): ログイン + タイムライン + 投稿作成

**ファイル:**
- 作成: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- 変更: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`
...
- [ ] **Step 2: Index.ets を書き直し（loginPanel/timelinePanel ビルダー）**
...
- [ ] **Step 3: 構文検証**（DevEco 環境が整った後）

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

期待結果: BUILD SUCCESSFUL（ローカルに DevEco/hvigor がない場合はスキップし、CI が整ったら検証）。
...
コミット: `git commit -m "chore: M1 closed-loop complete"`（変更が残っている場合）。

---

## Self-Review 記録

**1. Spec カバレッジ（§14 M1 行 + ユーザー追補）:**
- 登録/ログイン/プロフィール → T2、T3 ✓
- 投稿作成/詳細 → T4 ✓
- 簡易タイムライン → T4 ✓
- いいね・コメント → T5 ✓
- hg/apidoc インターフェースドキュメント（ユーザーの最新要望）→ T6 ✓（全コントローラに @Apidoc アノテーション付き）
- 3 つのモバイル端末すべてでログイン+タイムライン+投稿作成を実装 → T8/T9/T10 ✓

**2. プレースホルダースキャン:** TBD/TODO なし。admin AuthCheck ミドルウェア名は「異なる場合は既存のものを使う」と注記し、明確なフォールバックあり。

**3. 型の一貫性:**
- JwtHelper::encode/decode/revoke/isRevoked は T2 で定義され、T2 のテストで一貫して使用
- `$request->uid`/`$request->jti` は AuthMiddleware が注入。T3/T4/T5 のテストは `$req->uid` を直接代入 ✓
- レスポンスフィールド `access_token`/`refresh_token`/`expires_in` は T2 で snake_case と定義。T8/T9/T10 クライアントは SerialName/CodingKeys/リテラルで整合 ✓
- PostController::detail/like/unlike と CommentController::index/create は `string $id` を注入する同一シグネチャで、ルート `{id}` が対応 ✓

**4. 既知の制限（意図的な先送り）:**
- Redis がない場合、Redis ケースはスキップ（redisAvailable()）
- admin→service の gRPC 化は M2 へ延期（M1 はテーブル直接参照。テーブルプレフィックス social_ は検証済み）
- キャプチャ/画像アップロード/フォロー/完全フィード/翻訳 → M2+
- created_at の秒精度 → タイムラインテストの usleep(1100000) に注記済み
