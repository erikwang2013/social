...
**Goal:** 建立 monorepo 骨架、gRPC 契约与三端桩生成管线、四子系统可运行骨架、CI 全绿，并打通 service→infrastructure 端到端 gRPC 探活。

**Architecture:** 顶层目录 contracts/（proto 契约，唯一生成入口）→ buf 生成 PHP 桩（service、admin）与 Rust 桩（infrastructure）；service（webman v2）作为 gRPC 客户端，infrastructure（bee-rust + tonic）作为 gRPC 服务端；三端原生工程（iOS/Android/HarmonyOS）各自初始化并由 OpenAPI 生成客户端；GitHub Actions 矩阵 CI。

**Tech Stack:** PHP 8.3+ / webman v2 / grpc 扩展 / buf / protobuf / Rust（tonic + prost，bee-rust workspace）/ xcodegen / Gradle(Android) / hvigor(HarmonyOS) / GitHub Actions

**团队分工（对应设计文档 §16，M0 编制）：**
...
### Task 1: 仓库规范与根 README

**Files:**
- Create: `README.md`
- Modify: `.gitignore`

- [ ] **Step 1: 检查 .gitignore 覆盖**

```bash
grep -E 'vendor|node_modules|target|\.env|\.idea|build/' .gitignore
```
Expected: 每行都有匹配；缺失则补上（PHP `vendor/`、Rust `target/`、Android `build/`、Node `node_modules/`、`.env`）。
...
`contracts/user/user_service.proto`（service 对外服务，admin 调用；M0 仅探活桩）:
```proto
syntax = "proto3";
...
`contracts/admin/admin_service.proto`（admin 对外服务；M0 仅探活桩）:
```proto
syntax = "proto3";
...
```
Expected: 无输出错误，exit 0。若 buf 未安装：`go install github.com/bufbuild/buf/cmd/buf@latest` 或 `brew install buf`。
...
### Task 3: 桩生成管线 + PHP gRPC 就绪

**Files:**
- Create: `scripts/gen-contracts.sh`
- Create: `service/README.grpcs.md`（grpc 扩展安装说明）
...
```
Expected: `Social/Infra/V1/InfraServiceClient.php`、`Social/Common/V1/Pong.php` 等桩文件存在。

- [ ] **Step 3: PHP gRPC 依赖就绪（service 与 admin 各自执行）**
...
### Task 4: infrastructure tonic gRPC 服务骨架
...
### Task 5: service webman 骨架 + gRPC 探活客户端

**Files:**
- Create: `service/`（composer 生成 webman 工程）
...
```
Expected: `service/app/`、`service/config/`、`service/vendor/`、`service/start.php` 生成。
...
```
Expected: vendor 就绪；.env 按本地 MySQL/Redis 配置（不改动版本库内示例文件）。
...
```
Expected: 现有 open-admin 测试套件全绿（若工程无测试入口，记录到基线文档）。

- [ ] **Step 3: 写基线文档**

`docs/ADMIN_BASELINE.md`：记录 admin 当前版本、可运行状态、已启用模块（JWT/RBAC/审计/文件/i18n）、grpc 扩展就绪状态、后续改造入口（对应设计文档 §3.4 八项新增）。
...
> 本机为 Linux，无法构建 Xcode 工程；产出源码 + xcodegen 配置，构建验证放到 macOS CI（T10 预留 job，本任务不阻塞）。
...
`apps/ios/SocialApp/APIClient.swift`（网络层骨架，M1 接 OpenAPI 生成客户端）:
```swift
import Foundation
...
- [ ] **Step 4: 构建验证（需 Android SDK）**

```bash
cd apps/android && ./gradlew assembleDebug
```
Expected: BUILD SUCCESSFUL，产出 `app/build/outputs/apk/debug/app-debug.apk`。若本机无 SDK：记录环境要求，CI 验证。
...
### Task 9: HarmonyOS 工程初始化

> 本机若无 DevEco CLI 则产出工程结构 + 环境要求记录，构建验证后续 CI/DevEco 环境执行。
...
```
Expected: 输出 `E2E OK`（health + gRPC ping 均通过）。
...
- 覆盖：契约（T2/T3）→ 探活（T4/T5/T10）、三端初始化（T7/T8/T9）、admin 基线（T6）、CI（T10）——对应设计文档 M0 全部内容
- 占位符：无（所有步骤含真实命令与代码）
- 类型一致：PingRequest/Pong 三端桩一致；`pong from {client}` 断言在 T5/T10 统一
