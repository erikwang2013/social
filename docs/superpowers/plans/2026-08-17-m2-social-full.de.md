# M2 Vollständiger Social-Meilenstein-Implementierungsplan
...
**Goal:** Den M2-Meilenstein liefern: Follower-System, vollständiger Feed (Following-Feed), Volltextsuche (bee_search gRPC) und Benachrichtigungssystem (Like-/Kommentar-/Follow-Ereignisse).

**Architecture:** service (webman PHP) erhält Follow-/Notification-Modelle und -Controller; die Timeline wird zu „ich + Followees"; die Suche läuft per gRPC zu infrastructure (Rust tonic + bee_search SearchEngine, bei nicht verfügbarem ES degradiert service sauber mit 503); admin erhält Follow-Statistiken; Android/iOS/HarmonyOS erhalten Benutzerprofil- und Benachrichtigungsseiten.
...
### Task 1: Datenschicht — follows-/notifications-Tabellen und Modelle

**Files:**
- Create: `service/database/m2.sql`
- Create: `service/app/model/Follow.php`
- Create: `service/app/model/Notification.php`
- Modify: `service/tests/bootstrap.php:25-66` (Schema der beiden Tabellen anhängen)
- Create: `service/tests/FollowModelTest.php`

- [ ] **Step 1: m2.sql schreiben (Produktions-DB-Migration)**
...
- [ ] **Step 3: Testtabellen in bootstrap.php ergänzen**

Nach der Erstellung der `likes`-Tabelle in `service/tests/bootstrap.php` anhängen:
...
- [ ] **Step 4: FollowModelTest schreiben und Verifikation ausführen**
...
Ausführen: `cd /home/wwwroot/social/service && vendor/bin/phpunit tests/FollowModelTest.php`
Expected: 2 tests, PASS (die Verletzung der Unique-Constraint wirft QueryException, von expectException abgefangen).
...
- Modify: `service/config/route.php:41` (5 Routen in der AuthMiddleware-Gruppe ergänzen)
- Create: `service/tests/FollowControllerTest.php`

- [ ] **Step 1: UserBrief-Hilfsklasse**
...
(Prüfen, ob `app/model/User.php` eine `profile()`-Relation hat; falls nicht, `return $this->hasOne(UserProfile::class, 'user_id');` ergänzen)

- [ ] **Step 2: FollowController**

`service/app/controller/FollowController.php` (mit apidoc, im Stil von PostController):
...
Dem Follow-Modell zwei Relationen hinzufügen (nach FollowModelTest in `service/app/model/Follow.php` ergänzen):
...
Innerhalb der AuthMiddleware-Gruppe von `service/config/route.php` (vor `/posts`) anhängen:
...
`service/tests/FollowControllerTest.php` (Testmuster wie PostInteractionTest: `new Request('POST',...) + setPost + $req->uid + json_decode(rawBody)['code']`):
...
Hinweis: Die Methodensignatur des Controllers ist `follow(Request $request, string $id)`; Tests müssen `$ctrl->follow($req, '5')` direkt aufrufen und rawBody prüfen. Wenn der Controller `$request->uid` statt eines Attributs liest, im Test `$req->uid = $me->id;` verwenden (siehe PostInteractionTest-Muster — eine Variante wählen und konsistent bleiben).
...
### Task 3: Vollständiger Feed (Following-Feed)

**Files:**
- Modify: `service/app/controller/PostController.php:44-55` (Timeline wird zum Following-Feed)
- Create: `service/tests/PostFeedTest.php`

- [ ] **Step 1: Timeline umbauen**

Die `$paginator`-Zeile in `PostController::timeline` ändern zu:
...
(Rest unverändert; Dedupe von `$ids` kann entfallen — doppelte ids entfernt whereIn nach array_merge automatisch)
...
### Task 4: Benachrichtigungssystem

**Files:**
- Modify: `service/app/controller/PostController.php:90-95` (bei Like Benachrichtigung schreiben)
- Modify: `service/app/controller/CommentController.php:60-62` (bei Kommentar Benachrichtigung schreiben)
...
- [ ] **Step 1: Bei Like-/Comment-Ereignissen Benachrichtigungen schreiben**

Innerhalb des `if ($like->wasRecentlyCreated)`-Blocks in `PostController::like` anhängen:
...
Nach `$post->increment('comment_count');` in `CommentController::create` anhängen:
...
Innerhalb der AuthMiddleware-Gruppe von `service/config/route.php` anhängen:
...
### Task 5: Suchvertrag + Rust-Implementierung
...
`"../../../contracts/search/search_service.proto"` zum compile_protos-Array in `build.rs` hinzufügen.

Abhängigkeiten in `Cargo.toml` ergänzen:
...
`infrastructure/crates/bee_search/src/lib.rs` lesen; `StubEngine` aus `#[cfg(test)] mod tests` zu einem modulweiten `pub struct MemoryEngine` machen (alle Methoden des SearchEngine-Traits implementieren; Documents in einem In-Memory-HashMap speichern; search nutzt dieselbe Matching-Logik wie in tests — `contains` matcht id oder json). Die `#[cfg(test)]`-Beschränkung entfernen und oben mit `// ponytail: 内存实现，生产走 Elasticsearch feature` kennzeichnen. Mit `cargo build` verifizieren.
...
(`h.0.0`/`h.1.0` sind Tuple-Struct-Zugriffe — falls Document/DocumentId in bee_search Strukturen mit benannten Feldern sind, `.0.id`/`.0.content` verwenden und den Rückgabetyp von search prüfen; maßgeblich sind die tatsächlichen Definitionen in lib.rs. Falls SearchEngine::search `Vec<DocumentId>` statt Document zurückgibt, Hit.json als leeren String setzen oder zusätzlich fetchen.)

- [ ] **Step 5: In main.rs registrieren**
...
(Die Signatur von Elasticsearch::new richtet sich nach lib.rs; ist sie async, `.await` verwenden. Namenskonflikt-Handling: `use social::search::v1::search_service_server::SearchServiceServer as SearchServiceServerAlias`, um den Konflikt mit dem Namen des SearchService-Traits von tonic zu vermeiden.)
...
```
Expected: Build erfolgreich, `index_then_search_roundtrip` PASS.
...
### Task 6: Search-API in service (handgeschriebener PHP-gRPC-Stub)
...
- Modify: `service/app/controller/PostController.php:29-31` (Index nach create synchronisieren)
- Create: `service/tests/SearchControllerTest.php`

Hintergrund: grpc_php_plugin fehlt, Stub von Hand schreiben (nach dem Muster `service/generated/Social/Infra/V1/InfraServiceClient.php`).

- [ ] **Step 1: Message-Klassen (am Beispiel IndexRequest, Rest isomorph)**
...
`IndexResponse.php` / `DeleteRequest.php` sind isomorph. `SearchRequest.php` bekommt zwei `checkInt32`-Felder: `from`/`size`. `Hit.php` enthält `id`(int64)/`json`(string). Das repeated-Feld von `SearchResponse.php`:
...
- [ ] **Step 3: SearchSync-Singleton (fire-and-forget + Degradierung)**
...
- [ ] **Step 5: Routen + Post-Synchronisierung**
...
Nach `$post = Post::create(...)` in `PostController::create` anhängen:
...
### Task 7: Vollständige Verifikation + E2E-Smoke-Test

**Files:**
- Create: `service/scripts/probe_m2.php`

- [ ] **Step 1: Smoke-Test-Skript**

`service/scripts/probe_m2.php` (führt Eloquent + Controller direkt aus, kein Server nötig; `php scripts/probe_m2.php`):
...
> Hinweis: Das Probe kann einfach `require __DIR__ . '/../tests/bootstrap.php'` nutzen und so die Test-Tabellen wiederverwenden — kein erneutes Anlegen nötig. Dann die folgende Assertion-Kette ausführen:
> 1. 2 Benutzer A/B registrieren (AuthController::register oder direkt User::create + UserProfile::create)
> 2. A folgt B → prüfen, dass Follow existiert und B eine follow-Benachrichtigung erhalten hat
> 3. B postet → A's Timeline enthält den Beitrag (PostController::timeline)
> 4. A liked B's Beitrag → B hat eine like-Benachrichtigung erhalten
> 5. NotificationController::unreadCount > 0; nach readAll 0
> 6. SearchController::users('B's Spitzname') liefert einen Treffer
> 7. `M2 E2E OK` ausgeben, Exit-Code 0; jede fehlgeschlagene Assertion gibt einen Fehler aus und endet mit Exit-Code 1
...
### Task 8: Follow-Statistiken in admin

**Files:**
- Create: `admin/app/model/SocialFollow.php`
- Modify: `admin/app/admin/controller/UserController.php` (index um follows_count/fans_count erweitern)

- [ ] **Step 1: SocialFollow-Modell**

`admin/app/model/SocialFollow.php` (**zwingend mit Copyright-Header**):
...
- [ ] **Step 2: Statistiken in UserController**

Die index-Methode von `admin/app/admin/controller/UserController.php` lesen und in der Schleife/Mapping, die die Rückgabeliste aufbaut, ergänzen (zwei zusätzliche Felder pro Benutzerzeile):
...
(`use app\model\SocialFollow;` verwenden; wenn index ein paginiertes Array zurückgibt, an jedes item anhängen; weicht die Methodenstruktur ab, mit minimaler Änderung anpassen und die bestehende Rückgabestruktur beibehalten.)
...
### Task 9: Android-Client (Benutzerprofil + Benachrichtigungen)

**Files:**
- Modify: `apps/android/.../api/ApiClient.kt` (BASE 8787→8788 + neue API-Methoden)
- Modify: `apps/android/.../MainActivity.kt` (Einstiegspunkte)
- Create: `apps/android/.../api/UserScreen.kt`
- Create: `apps/android/.../api/NotificationScreen.kt`

- [ ] **Step 1: BASE korrigieren und API-Methoden ergänzen**

In `ApiClient.kt` `BASE = "http://10.0.2.2:8787"` zu `"http://10.0.2.2:8788"` ändern. Data Classes und Methoden ergänzen (im bestehenden synchronen OkHttp- + kotlinx.serialization-Muster):
...
(`post`/`get`/`getList` nutzen die bestehenden privaten Methodensignaturen wieder; `getList` parst das Feld `data.list`, Feldnamen wie beim bestehenden TimelineScreen-Parsing.)

- [ ] **Step 2: UserScreen**

`UserScreen.kt`: nach dem Aufbau von `TimelineScreen.kt` (ListView + Singleton ApiClient + Executor). Anzeigen: Avatar, Spitzname, bio, Follower-/Following-Zahlen und Follow-Button; darunter eine Liste „seine/ihre Beiträge" (Layout des Beitrags-Items aus TimelineScreen wiederverwenden; falls `GET /api/v1/users/{id}/posts` nicht verfügbar ist, Timeline aufrufen und clientseitig filtern oder verstecken — Priorität ist die Follow-Interaktion + Relationsanzeige; für die Beitragsliste die vollständige Timeline-API nutzen).

> Hinweis: Falls die Route `GET /api/v1/users/{id}/posts` nicht existiert, kann der Client die bestehende Liste von `GET /posts` verwenden; kein neuer Backend-Endpunkt.

- [ ] **Step 3: NotificationScreen**

`NotificationScreen.kt`: ListView mit `NotificationItem` (chinesische Labels für type: like→"赞了你的动态", comment→"评论了你的动态", follow→"关注了你"), Klick führt zu MarkAllRead.

- [ ] **Step 4: Einstiegspunkte in MainActivity**

In `MainActivity.kt` zwei Einstiegspunkte ergänzen (Benutzer-Avatar / Benachrichtigungs-Icon führen zum jeweiligen Screen, im bestehenden Intent/Layout-Navigationsstil).

- [ ] **Step 5: Kompilierungsprüfung**
...
- [ ] **Step 1: Zuerst bestehenden Code lesen**

Die vorhandenen Dateien in `apps/ios/` und `apps/harmonyos/` lesen und das Netzwerkschicht-Muster (Request-Wrapper in APIClient.swift, `@ohos.net.http`-Wrapper in ApiService.ets) sowie die Seitenstruktur bestätigen.

- [ ] **Step 2: iOS — APIs und Seiten ergänzen**

In `APIClient.swift` ergänzen (im bestehenden Methodenstil): `follow(id:)`, `unfollow(id:)`, `relation(id:)`, `notifications()`, `unreadCount()`, `markAllRead()`. In `ContentView.swift` zwei View-Einstiegspunkte ergänzen: Benutzerprofil (Follow-Button + Follower-/Following-Zahlen) und Benachrichtigungsliste (chinesische type-Labels + Ungelesen-Badge).

- [ ] **Step 3: HarmonyOS — APIs und Seiten ergänzen**

In `ApiService.ets` dieselben Methoden ergänzen (`follow`/`unfollow`/`relation`/`notifications`/`unreadCount`/`markAllRead`). In `Index.ets` Benutzerprofil- und Benachrichtigungsseite ergänzen (im bestehenden List/Scroll-Layout + State-Management-Stil).

- [ ] **Step 4: Syntaxprüfung**

HarmonyOS: `cd apps/harmonyos && hvigorw assembleHap` ausführen, falls die Umgebung verfügbar ist, sonst die `node --check`-artige Syntaxprüfung überspringen und als ungeprüft kennzeichnen.
iOS: Wenn auf diesem Rechner kein Xcode vorhanden ist, als nicht kompiliert/geprüft kennzeichnen.
...
1. Alle `service`-PHPUnit-Tests bestehen (inkl. der 4 neuen Testdateien)
2. `infrastructure` cargo build + cargo test bestehen (inkl. Roundtrip-Test)
3. `probe_m2.php` gibt `M2 E2E OK` aus
4. admin `php -l` besteht
5. Android compileDebugKotlin besteht
6. Bestehende M0/M1-Tests ohne Regression
7. Alle Commits sind gepusht (gemäß dem etablierten Workflow des Benutzers)
