...
**Goal:** M1 클로즈드 루프 완성: 회원가입/로그인/프로필, 게시물 작성/상세, 단순 타임라인, 좋아요·댓글, 그리고 service의 모든 API에 대한 hg/apidoc 인터페이스 문서 생성.

**Architecture:** service(webman)가 모든 사용자용 HTTP API(`/api/v1`)를 제공하며, JWT 이중 토큰 인증(access 2h + refresh 14d, 로그아웃은 jti Redis 블랙리스트), Eloquent가 MySQL에 직접 연결(테이블 접두사 `social_`); admin은 사용자 관리를 담당(M1은 테이블 직접 조회, gRPC 전환은 M2로 연기); 세 모바일 클라이언트(Android/iOS/HarmonyOS)가 로그인 + 타임라인 + 게시물 작성을 네이티브로 구현. 모든 service API는 hg/apidoc 어노테이션으로 인터페이스 문서를 생성합니다.

**기술 스택:** PHP 8.3 / webman 2.x / webman/database (Eloquent) / phpunit / grpc ext / hg/apidoc / JWT (erikwang2013/jwt-webman) / Redis / Kotlin+OkHttp / Swift+URLSession / ArkTS+http module

**알려진 제한(연기 항목):** 인증번호, 이미지 업로드, 팔로우, 전체 피드, admin→service gRPC 전환, 콘텐츠 번역 → 전부 M2+에서 처리.
...
### Task 1 (backend-service): 데이터베이스 계층 + 테스트 골격

**파일:**
- 생성: `service/config/database.php`
- 생성: `service/database/migrations/m1.sql`
- 생성: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- 생성: `service/phpunit.xml`
- 생성: `service/tests/bootstrap.php`
- 생성: `service/tests/AuthModelTest.php`
- 수정: `service/composer.json`(webman/database + phpunit dev)
...
- [ ] **Step 2: config/database.php 작성**（admin/config/database.php 참고: default는 mysql, test 연결은 sqlite :memory:）
...
- [ ] **Step 3: m1.sql 작성**（구조만 포함, 운영/로컬 mysql용; 테스트는 bootstrap에서 인라인으로 테이블 생성）
...
- [ ] **Step 4: 모델 작성**（모델 5개, Eloquent는 config('database')에서 prefix를 읽음）
...
- [ ] **Step 5: phpunit.xml + tests/bootstrap.php 작성**
...
- [ ] **Step 6: 테스트 tests/AuthModelTest.php 작성**
...
### Task 2 (backend-service): 인증（회원가입/로그인/갱신/로그아웃/me）

**파일:**
- 생성: `service/app/common/JwtHelper.php`
- 생성: `service/app/middleware/AuthMiddleware.php`
- 생성: `service/app/controller/AuthController.php`
- 생성: `service/config/route.php`
- 생성: `service/config/plugin/erikwang2013/jwt/jwt.php`
- 생성: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- 수정: `service/app/functions.php`(redisAvailable() 추가)

- [ ] **Step 1: JWT 패키지 설치（composer.json에 이미 있으면 건너뜀）**
...
- [ ] **Step 3: app/functions.php에 redisAvailable() 추가**（테스트에서 Redis 관련 케이스를 건너뛰기 위함）
...
> 참고: erikwang2013/jwt-webman의 JWTFactory::createFromConfig도 사용 가능합니다. 테스트를 단순하게 유지하기 위해(내부 구조에 의존하지 않음) 하위 레이어인 firebase/php-jwt를 직접 사용합니다. 패키지에 firebase/jwt가 없으면 `erikwang2013\jwt\JWTFactory` 방식으로 대체:
...
- [ ] **Step 6: AuthController**（@Apidoc 어노테이션 포함——Task 6에서 apidoc 생성을 활성화하며, 여기서 바로 작성）
...
- [ ] **Step 7: 라우트 config/route.php**（Task 6에서 apidoc register를 추가함）
...
- [ ] **Step 8: 테스트 tests/AuthControllerTest.php**
...
- [ ] **Step 9: 테스트 tests/AuthFlowTest.php**（Redis 의존 케이스는 redisAvailable()일 때만 실행）
...
예상: 모두 PASS（Redis 케이스는 건너뛰거나 통과）。
...
### Task 3 (backend-service): 프로필 API（me 업데이트）

**파일:**
- 생성: `service/app/controller/MeController.php`
- 생성: `service/tests/MeControllerTest.php`
...
### Task 4 (backend-service): 게시물（작성/타임라인/상세）

**파일:**
- 생성: `service/app/controller/PostController.php`
- 생성: `service/tests/PostControllerTest.php`

- [ ] **Step 1: PostController 작성（create/timeline/detail）**
...
- [ ] **Step 2: PostControllerTest 작성**（timeline 정렬 테스트는 created_at 초 단위 정밀도를 구분하기 위해 usleep(1100000) 필요）
...
예상: 모두 PASS（timeline 테스트 약 2-3초）。
...
### Task 5 (backend-service): 좋아요 + 댓글

**파일:**
- 수정: `service/app/controller/PostController.php`(like/unlike 추가)
- 생성: `service/app/controller/CommentController.php`
- 생성: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: PostController에 like/unlike 추가**
...
### Task 6 (backend-service): hg/apidoc 인터페이스 문서

**파일:**
- 생성: `service/config/plugin/hg/apidoc/app.php`
- 생성: `service/config/plugin/hg/apidoc/route.php`
- 생성: `service/app/common/Definitions.php`
- 수정: `service/config/route.php`(apidoc 라우트 등록)

- [ ] **Step 1: hg/apidoc 설치**（composer.json에 "hg/apidoc": "*"가 이미 있으므로 바로 update）

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: apidoc 설정 작성**（admin 템플릿 복사 후 수정）
...
- [ ] **Step 3: Definitions.php 작성（공통 응답 구조）**
...
- [ ] **Step 4: apidoc 라우트 등록（config/route.php 끝에 추가）**
...
예상: `apidoc OK` 출력（페이지 제목: Social 사용자 API）。

- [ ] **Step 6: 전체 테스트 실행으로 회귀 없음 확인**
...
### Task 7 (backend-admin): 사용자 관리（social_ 테이블 직접 조회）

**파일:**
- 생성: `admin/app/model/SocialUser.php`
- 생성: `admin/app/admin/controller/SocialUserController.php`
- 수정: `admin/config/route.php`(/admin/user 라우트 그룹)

- [ ] **Step 1: SocialUser 모델 작성**
...
- [ ] **Step 2: SocialUserController 작성**
...
- [ ] **Step 3: admin 라우트 추가（config/route.php 끝）**
...
> 참고: AuthCheck는 open-admin에 이미 있는 미들웨어입니다(이름이 다르면 admin의 기존 로그인 검증 미들웨어 사용). M2에서 gRPC로 service를 호출하도록 변경됩니다.
...
예상: `No syntax errors detected` 4줄.
...
### Task 8 (android-dev): 로그인 + 타임라인 + 게시물 작성

**파일:**
- 생성: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- 생성: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- 생성: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
- 수정: `apps/android/app/src/main/java/.../MainActivity.kt`
- 수정: `apps/android/app/build.gradle.kts`(okhttp + kotlinx-serialization)
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt**（Compose 또는 네이티브 View——앱의 기존 UI 스타일을 따름; 여기서는 네이티브 View로 최소 구현）
...
### Task 9 (ios-dev): 로그인 + 타임라인 + 게시물 작성

**파일:**
- 수정: `apps/ios/.../APIClient.swift`(재작성)
- 수정: `apps/ios/.../ContentView.swift`(LoginView + TimelineView)
...
> 참고: iOS 프로젝트가 storyboard/UIKit을 사용한다면 ContentView를 동등한 UIKit 컨트롤러로 교체하세요. 여기서는 SwiftUI 템플릿 기준으로 제공합니다.
...
### Task 10 (harmonyos-dev): 로그인 + 타임라인 + 게시물 작성

**파일:**
- 생성: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
- 수정: `apps/harmonyos/entry/src/main/ets/pages/Index.ets`
...
- [ ] **Step 2: Index.ets 재작성（loginPanel/timelinePanel 빌더）**
...
- [ ] **Step 3: 문법 검증**（DevEco 환경 준비 후）

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

예상: BUILD SUCCESSFUL（로컬에 DevEco/hvigor가 없으면 건너뛰고 CI 준비 후 검증）。
...
커밋: `git commit -m "chore: M1 closed-loop complete"`(남은 변경 사항이 있을 경우).

---

## Self-Review 기록

**1. Spec coverage（§14 M1 항목 + 사용자 보충）：**
- 회원가입/로그인/프로필 → T2, T3 ✓
- 게시물 작성/상세 → T4 ✓
- 단순 타임라인 → T4 ✓
- 좋아요·댓글 → T5 ✓
- hg/apidoc 인터페이스 문서（사용자 최신 요구）→ T6 ✓（모든 controller에 @Apidoc 어노테이션）
- 세 모바일 클라이언트 모두 로그인+타임라인+게시물 작성 구현 → T8/T9/T10 ✓

**2. Placeholder 스캔:** TBD/TODO 없음; admin AuthCheck 미들웨어 이름에 "다르면 기존 것을 사용"이라고 명시되어 있어 명확한 fallback 존재.

**3. 타입 일관성：**
- JwtHelper::encode/decode/revoke/isRevoked는 T2에서 정의하고 T2 테스트에서 일관되게 사용
- `$request->uid`/`$request->jti`는 AuthMiddleware가 주입하며, T3/T4/T5 테스트는 `$req->uid`를 직접 할당 ✓
- 응답 필드 `access_token`/`refresh_token`/`expires_in` 스네이크케이스 명명은 T2에서 정의, T8/T9/T10 클라이언트는 SerialName/CodingKeys/리터럴로 정렬 ✓
- PostController::detail/like/unlike와 CommentController::index/create 메서드가 `string $id` 주입 시그니처로 일치하며, 라우트 `{id}`가 대응 ✓

**4. 알려진 제한（의도적 연기）：**
- Redis가 없으면 Redis 케이스 건너뜀（redisAvailable()）
- admin→service gRPC 전환은 M2로 연기（M1은 테이블 직접 조회, 테이블 접두사 social_ 검증 완료）
- 인증번호/이미지 업로드/팔로우/전체 피드/번역 → M2+
- created_at 초 단위 정밀도 → timeline 테스트 usleep(1100000)으로 명시됨
