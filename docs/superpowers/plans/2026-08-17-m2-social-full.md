# M2 社交完整里程碑 Implementation Plan
...
**Goal:** 交付 M2 里程碑：关注体系、完整信息流（关注流）、全文搜索（bee_search gRPC）、通知体系（点赞/评论/关注事件）。

**Architecture:** service（webman PHP）新增 Follow/Notification 模型与控制器，时间线改为"自己+关注者"；搜索走 gRPC 到 infrastructure（Rust tonic + bee_search SearchEngine，ES 不可用时 service 优雅降级 503）；admin 增加关注统计；Android/iOS/HarmonyOS 增加用户主页与通知页。

**Tech Stack:** PHP 8.3 / webman 2.x / Eloquent / sqlite :memory: 测试；Rust tonic 0.12 + prost 0.13 + bee_search（feature "elasticsearch"）；gRPC proto；Android OkHttp + kotlinx.serialization。

---

### Task 1: 数据层 — follows / notifications 表与模型

**Files:**
- Create: `service/database/m2.sql`
- Create: `service/app/model/Follow.php`
- Create: `service/app/model/Notification.php`
- Modify: `service/tests/bootstrap.php:25-66`（追加两张表 schema）
- Create: `service/tests/FollowModelTest.php`

- [ ] **Step 1: 写 m2.sql（生产库迁移）**
...
- [ ] **Step 3: bootstrap.php 追加测试表**

在 `service/tests/bootstrap.php` 的 `likes` 表创建之后追加：
...
- [ ] **Step 4: 写 FollowModelTest 并运行验证**
...
运行：`cd /home/wwwroot/social/service && vendor/bin/phpunit tests/FollowModelTest.php`
Expected: 2 tests, PASS（唯一约束违反抛 QueryException 被 expectException 捕获）。
...
- Modify: `service/config/route.php:41`（AuthMiddleware group 内加 5 条路由）
...
（确认 `app/model/User.php` 有 `profile()` 关系；若无则加 `return $this->hasOne(UserProfile::class, 'user_id');`）

- [ ] **Step 2: FollowController**

`service/app/controller/FollowController.php`（含 apidoc，遵循 PostController 风格）：
...
Follow 模型补充两个关系（FollowModelTest 后追加到 `service/app/model/Follow.php`）：
...
`service/config/route.php` 的 AuthMiddleware group 内（`/posts` 之前）追加：
...
`service/tests/FollowControllerTest.php`（测试模式沿用 PostInteractionTest：`new Request('POST',...) + setPost + $req->uid + json_decode(rawBody)['code']`）：
...
注意：控制器方法签名 `follow(Request $request, string $id)`，测试需直接调用 `$ctrl->follow($req, '5')` 并断言 rawBody。若控制器用 `$request->uid` 而非 attribute，测试中 `$req->uid = $me->id;`（参考 PostInteractionTest 模式，二选一保持一致）。
...
**Files:**
- Modify: `service/app/controller/PostController.php:44-55`（timeline 改为关注流）
- Create: `service/tests/PostFeedTest.php`

- [ ] **Step 1: 改造 timeline**

`PostController::timeline` 中 `$paginator` 一行改为：
...
（其余不动；`$ids` 去重可省——array_merge 后重复 id 由 whereIn 天然去重）
...
**Files:**
- Modify: `service/app/controller/PostController.php:90-95`（like 时写通知）
- Modify: `service/app/controller/CommentController.php:60-62`（评论时写通知）
...
- [ ] **Step 1: like/comment 事件写通知**

`PostController::like` 中 `if ($like->wasRecentlyCreated)` 块内追加：
...
`CommentController::create` 中 `$post->increment('comment_count');` 之后追加：
...
`service/config/route.php` AuthMiddleware group 内追加：
...
`build.rs` 的 compile_protos 数组追加 `"../../../contracts/search/search_service.proto"`。
...
读 `infrastructure/crates/bee_search/src/lib.rs`，把 `#[cfg(test)] mod tests` 内的 `StubEngine` 提升为模块级 `pub struct MemoryEngine`（实现 SearchEngine trait 的全部方法；在内存 HashMap 存 Document，search 用与 tests 中相同的匹配逻辑——`contains` 匹配 id 或 json）。删除 `#[cfg(test)]` 限制，并在其顶部标注 `// ponytail: 内存实现，生产走 Elasticsearch feature`。`cargo build` 验证通过。
...
（`h.0.0`/`h.1.0` 是 tuple struct 访问——若 bee_search 的 Document/DocumentId 是 named field 结构体，改为 `.0.id`/`.0.content` 并核对 search 返回类型；以 lib.rs 实际定义为准。若 SearchEngine::search 返回 `Vec<DocumentId>` 而非 Document，则调整 Hit.json 为空串或额外 fetch。）
...
（Elasticsearch::new 的签名以 lib.rs 为准；若为 async 则 `.await`。冲突处理：`use social::search::v1::search_service_server::SearchServiceServer as SearchServiceServerAlias` 避免与 tonic 的 SearchService trait 名冲突。）
...
```
Expected: build 通过，`index_then_search_roundtrip` PASS。
...
### Task 6: service 搜索 API（手写 PHP gRPC stub）
...
- Modify: `service/app/controller/PostController.php:29-31`（create 后同步索引）
- Create: `service/tests/SearchControllerTest.php`

背景：grpc_php_plugin 缺失，手写 stub（照 `service/generated/Social/Infra/V1/InfraServiceClient.php` 模式）。

- [ ] **Step 1: message 类（以 IndexRequest 为例，其余同构）**
...
`IndexResponse.php` / `DeleteRequest.php` 同构。`SearchRequest.php` 加 `from`/`size` 两个 `checkInt32` 字段。`Hit.php` 含 `id`(int64)/`json`(string)。`SearchResponse.php` 的 repeated 字段：
...
- [ ] **Step 3: SearchSync 单例（fire-and-forget + 降级）**
...
`PostController::create` 中 `$post = Post::create(...)` 之后追加：
...
`service/scripts/probe_m2.php`（直接跑 Eloquent + 控制器，无需起服务；`php scripts/probe_m2.php`）：
...
> 提示：probe 直接 `require __DIR__ . '/../tests/bootstrap.php'` 复用测试建表即可，无需重复建表。然后按以下断言链执行：
> 1. 注册 2 用户 A/B（AuthController::register，或直接 User::create + UserProfile::create）
> 2. A 关注 B → 断言 Follow 存在、B 收到 follow 通知
> 3. B 发帖 → A 的 timeline 含该帖（PostController::timeline）
> 4. A 点赞 B 的帖 → B 收到 like 通知
> 5. NotificationController::unreadCount 断言 > 0；readAll 后为 0
> 6. SearchController::users('B昵称') 断言命中
> 7. 输出 `M2 E2E OK`，退出码 0；任何断言失败输出错误并退出码 1
...
- Modify: `admin/app/admin/controller/UserController.php`（index 加 follows_count/fans_count）

- [ ] **Step 1: SocialFollow 模型**

`admin/app/model/SocialFollow.php`（**必须带版权头**）：
...
- [ ] **Step 2: UserController 统计**

读 `admin/app/admin/controller/UserController.php` 的 index 方法，在返回列表的循环/映射中追加（每行用户加两个字段）：
...
（用 `use app\model\SocialFollow;`；若 index 是分页数组返回，追加到 item 上；若该方法结构不同，按最小改动适配，保持现有返回结构。）
...
### Task 9: Android 客户端（用户主页 + 通知）

**Files:**
- Modify: `apps/android/.../api/ApiClient.kt`（BASE 8787→8788 + 新 API 方法）
- Modify: `apps/android/.../MainActivity.kt`（入口）
- Create: `apps/android/.../api/UserScreen.kt`
- Create: `apps/android/.../api/NotificationScreen.kt`

- [ ] **Step 1: 修 BASE 并加 API 方法**

`ApiClient.kt` 中 `BASE = "http://10.0.2.2:8787"` 改为 `"http://10.0.2.2:8788"`。追加 data class 与方法（沿用现有 OkHttp 同步 + kotlinx.serialization 模式）：
...
（`post`/`get`/`getList` 复用现有私有方法签名；`getList` 解析 `data.list` 字段，字段名以现有 TimelineScreen 解析方式为准。）

- [ ] **Step 2: UserScreen**

`UserScreen.kt`：仿 `TimelineScreen.kt` 结构（ListView + 单例 ApiClient + Executor）。展示：头像、昵称、bio、粉丝数/关注数/是否已关注按钮；下方"TA 的动态"列表（复用 TimelineScreen 的帖子 item 布局，改调 `GET /api/v1/users/{id}/posts` 不可用则改调 timeline 后客户端过滤，或直接隐藏——优先只做关注交互 + 关系展示，动态列表用 timeline 全量接口）。

> 注：若 `GET /api/v1/users/{id}/posts` 路由不存在，客户端用现有 `GET /posts` 的列表即可，不新增后端接口。

- [ ] **Step 3: NotificationScreen**

`NotificationScreen.kt`：ListView 展示 `NotificationItem`（type 中文标签：like→"赞了你的动态"、comment→"评论了你的动态"、follow→"关注了你"），点击进入 MarkAllRead。

- [ ] **Step 4: MainActivity 入口**

`MainActivity.kt` 增加两个入口（用户头像/通知图标跳转对应 Screen，沿用现有 Intent/布局跳转方式）。
...
读 `apps/ios/` 与 `apps/harmonyos/` 现有文件，确认网络层模式（APIClient.swift 的请求封装、ApiService.ets 的 `@ohos.net.http` 封装）与页面组织方式。

- [ ] **Step 2: iOS 追加 API + 页面**

`APIClient.swift` 追加（沿用现有方法模式）：`follow(id:)`、`unfollow(id:)`、`relation(id:)`、`notifications()`、`unreadCount()`、`markAllRead()`。`ContentView.swift` 增加用户主页（关注按钮 + 粉丝/关注数）与通知列表（type 中文标签 + 未读角标）两个视图入口。

- [ ] **Step 3: HarmonyOS 追加 API + 页面**

`ApiService.ets` 追加同类方法（`follow`/`unfollow`/`relation`/`notifications`/`unreadCount`/`markAllRead`）。`Index.ets` 增加用户主页与通知页（沿用现有 List/Scroll 布局 + 状态管理方式）。

- [ ] **Step 4: 语法检查**

HarmonyOS：`cd apps/harmonyos && hvigorw assembleHap` 若环境可用则跑，否则 `node --check` 类语法检查跳过，注明未验证。
iOS：本机无 Xcode 时注明未编译验证。
...
1. `service` 全部 PHPUnit 测试通过（含新增 4 个测试文件）
2. `infrastructure` cargo build + cargo test 通过（含 roundtrip 测试）
3. `probe_m2.php` 输出 `M2 E2E OK`
4. admin `php -l` 通过
5. Android compileDebugKotlin 通过
6. M0/M1 既有测试无回归
7. 所有提交已 push（用户既定工作流）
