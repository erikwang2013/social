# Plano de implementação do marco M1 em loop fechado
...
**Objetivo:** concluir o loop fechado de M1: registro/login/perfil, publicação de post/detalhe, linha do tempo simplificada, curtidas e comentários, e gerar a documentação de interface hg/apidoc para todas as APIs do service.

**Arquitetura:** o service (webman) fornece todas as APIs HTTP do lado do cliente (`/api/v1`) com autenticação JWT de token duplo (access 2 h + refresh 14 d, jti Redis blacklist para logout), Eloquent conectado diretamente ao MySQL (prefixo de tabela `social_`); o admin fornece a gestão de usuários (em M1, consulta direta às tabelas, gRPC adiado para M2); os três clientes móveis (Android/iOS/HarmonyOS) implementam nativamente login + linha do tempo + publicar post. Todas as APIs do service usam anotações hg/apidoc para gerar a documentação.
...
**Limitações conhecidas (adiadas):** captcha, upload de imagens, seguir, feed completo, gRPC admin→service, tradução de conteúdo → tudo em M2+.
...
### Task 1 (backend-service): camada de banco de dados + esqueleto de testes

**Arquivos:**
- Criar: `service/config/database.php`
- Criar: `service/database/migrations/m1.sql`
- Criar: `service/app/model/User.php`, `UserProfile.php`, `Post.php`, `Comment.php`, `Like.php`
- Criar: `service/phpunit.xml`
- Criar: `service/tests/bootstrap.php`
- Criar: `service/tests/AuthModelTest.php`
...
- [ ] **Step 2: escrever config/database.php** (imitando admin/config/database.php: default mysql, conexão de teste com sqlite :memory:)
...
- [ ] **Step 3: escrever m1.sql** (somente estrutura, para mysql de produção/local; os testes criam as tabelas inline via bootstrap)
...
- [ ] **Step 4: escrever os modelos** (5 modelos; o Eloquent lê o prefixo de config('database'))
...
- [ ] **Step 5: escrever phpunit.xml + tests/bootstrap.php**
...
- [ ] **Step 6: escrever o teste tests/AuthModelTest.php**
...
### Task 2 (backend-service): autenticação (registro/login/refresh/logout/me)

**Arquivos:**
- Criar: `service/app/common/JwtHelper.php`
- Criar: `service/app/middleware/AuthMiddleware.php`
- Criar: `service/app/controller/AuthController.php`
- Criar: `service/config/route.php`
- Criar: `service/config/plugin/erikwang2013/jwt/jwt.php`
- Criar: `service/tests/AuthControllerTest.php`, `service/tests/AuthFlowTest.php`
- Modificar: `service/app/functions.php` (adicionar redisAvailable())

- [ ] **Step 1: instalar o pacote JWT (pular se já estiver no composer.json)**

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: configuração do jwt.php**
...
- [ ] **Step 3: adicionar redisAvailable() a app/functions.php** (para que os testes possam pular os casos que dependem de Redis)
...
> Observação: JWTFactory::createFromConfig do erikwang2013/jwt-webman também serve; para manter os testes simples (sem depender de seus internals), usamos diretamente sua camada subjacente firebase/php-jwt. Se o pacote não trouxer firebase/jwt, usar a variante `erikwang2013\jwt\JWTFactory`:
...
- [ ] **Step 6: AuthController** (com anotações @Apidoc — a Task 6 habilitará a geração de apidoc, escrevê-las aqui diretamente)
...
- [ ] **Step 7: rotas config/route.php** (a Task 6 acrescentará o registro do apidoc)
...
- [ ] **Step 8: teste tests/AuthControllerTest.php**
...
- [ ] **Step 9: teste tests/AuthFlowTest.php** (os casos que dependem de Redis só são executados quando redisAvailable())
...
- [ ] **Step 10: Executar os testes**

```bash
cd service && vendor/bin/phpunit
```

Esperado: tudo PASS (casos Redis pulados ou aprovados).
...
### Task 3 (backend-service): API de perfil (atualização de me)

**Arquivos:**
- Criar: `service/app/controller/MeController.php`
- Criar: `service/tests/MeControllerTest.php`

- [ ] **Step 1: escrever MeController**
...
- [ ] **Step 2: escrever MeControllerTest**
...
### Task 4 (backend-service): posts (publicação/linha do tempo/detalhe)

**Arquivos:**
- Criar: `service/app/controller/PostController.php`
- Criar: `service/tests/PostControllerTest.php`

- [ ] **Step 1: escrever PostController (create/timeline/detail)**
...
- [ ] **Step 2: escrever PostControllerTest** (o teste de ordenação da linha do tempo precisa de usleep(1100000) porque created_at tem precisão de segundos)
...
Esperado: tudo PASS (teste da linha do tempo leva cerca de 2-3 s).
...
### Task 5 (backend-service): curtidas + comentários

**Arquivos:**
- Modificar: `service/app/controller/PostController.php` (adicionar like/unlike)
- Criar: `service/app/controller/CommentController.php`
- Criar: `service/tests/PostInteractionTest.php`

- [ ] **Step 1: adicionar like/unlike ao PostController**
...
- [ ] **Step 2: escrever CommentController**
...
- [ ] **Step 3: escrever PostInteractionTest**
...
### Task 6 (backend-service): documentação de interface hg/apidoc

**Arquivos:**
- Criar: `service/config/plugin/hg/apidoc/app.php`
- Criar: `service/config/plugin/hg/apidoc/route.php`
- Criar: `service/app/common/Definitions.php`
- Modificar: `service/config/route.php` (registrar as rotas do apidoc)

- [ ] **Step 1: instalar hg/apidoc** (composer.json já tem "hg/apidoc": "*", basta dar update)

```bash
cd service && composer update --no-interaction --no-plugins
```

- [ ] **Step 2: escrever a configuração do apidoc** (copiar o modelo do admin e modificar)
...
- [ ] **Step 3: escrever Definitions.php (estrutura de resposta comum)**
...
- [ ] **Step 4: registrar as rotas do apidoc (adicionar ao final de config/route.php)**
...
- [ ] **Step 5: verificar a geração localmente**
...
Esperado: saída `apidoc OK` (título da página: API do cliente Social).

- [ ] **Step 6: executar todos os testes para confirmar que não há regressões**
...
### Task 7 (backend-admin): gestão de usuários (consulta direta às tabelas social_)

**Arquivos:**
- Criar: `admin/app/model/SocialUser.php`
- Criar: `admin/app/admin/controller/SocialUserController.php`
- Modificar: `admin/config/route.php` (grupo de rotas /admin/user)

- [ ] **Step 1: escrever o modelo SocialUser**
...
- [ ] **Step 2: escrever SocialUserController**
...
- [ ] **Step 3: adicionar as rotas do admin (final de config/route.php)**
...
> Observação: AuthCheck é um middleware existente do open-admin (se o nome for diferente, usar o middleware de verificação de login existente do admin). Em M2, isso será alterado para chamadas gRPC ao service.
...
Esperado: 4 linhas de `No syntax errors detected`.
...
### Task 8 (android-dev): login + linha do tempo + publicar post

**Arquivos:**
- Criar: `apps/android/app/src/main/java/.../api/ApiClient.kt`
- Criar: `apps/android/app/src/main/java/.../api/LoginScreen.kt`
- Criar: `apps/android/app/src/main/java/.../api/TimelineScreen.kt`
...
- [ ] **Step 1: adicionar dependências ao build.gradle.kts**
...
- [ ] **Step 3: LoginScreen.kt + TimelineScreen.kt + MainActivity.kt** (Compose ou Views nativas — seguindo o estilo de UI existente do app; aqui uma implementação mínima com Views nativas)
...
### Task 9 (ios-dev): login + linha do tempo + publicar post

**Arquivos:**
- Modificar: `apps/ios/.../APIClient.swift` (reescrever)
...
> Observação: se o projeto iOS usar storyboard/UIKit, substitua ContentView por um controlador UIKit equivalente; aqui seguimos o modelo SwiftUI.
...
### Task 10 (harmonyos-dev): login + linha do tempo + publicar post

**Arquivos:**
- Criar: `apps/harmonyos/entry/src/main/ets/service/ApiService.ets`
...
- [ ] **Step 2: reescrever Index.ets (builders loginPanel/timelinePanel)**
...
- [ ] **Step 3: verificação de sintaxe** (quando o ambiente DevEco estiver pronto)

```bash
cd apps/harmonyos && hvigorw assembleHap --mode module -p module=entry@default 2>&1 | tail -5
```

Esperado: BUILD SUCCESSFUL (se não houver DevEco/hvigor na máquina, pular e verificar quando a CI estiver pronta).
...
## Verificação final (após a conclusão de todas as tarefas)
...
Confirmar: `git commit -m "chore: M1 closed-loop complete"` (se houver alterações restantes).

---

## Registro de Self-Review

**1. Cobertura do spec (linhas §14 de M1 + acréscimos do usuário):**
- Registro/login/perfil → T2, T3 ✓
- Publicação de post/detalhe → T4 ✓
- Linha do tempo simplificada → T4 ✓
- Curtidas e comentários → T5 ✓
- Documentação de interface hg/apidoc (último requisito do usuário) → T6 ✓ (todos os controllers têm anotações @Apidoc)
- Os três clientes móveis implementam login+linha do tempo+publicar post → T8/T9/T10 ✓

**2. Varredura de placeholders:** sem TBD/TODO; o nome do middleware AuthCheck do admin indica "se for diferente, usar o existente", com fallback claro.

**3. Consistência de tipos:**
- JwtHelper::encode/decode/revoke/isRevoked são definidos em T2, e os testes de T2 os usam de forma consistente
- `$request->uid`/`$request->jti` são injetados pelo AuthMiddleware; os testes T3/T4/T5 atribuem `$req->uid` diretamente ✓
- Os campos de resposta `access_token`/`refresh_token`/`expires_in` em snake_case são definidos em T2; os clientes T8/T9/T10 se alinham via SerialName/CodingKeys/literais ✓
- PostController::detail/like/unlike e CommentController::index/create injetam `string $id` com assinaturas idênticas; a rota `{id}` corresponde ✓

**4. Limitações conhecidas (adiadas propositalmente):**
- Os casos Redis são pulados quando não há Redis (redisAvailable())
- gRPC admin→service adiado para M2 (M1 consulta as tabelas diretamente; prefixo social_ verificado)
- Captcha/upload de imagens/seguir/feed completo/tradução → M2+
- Precisão de segundos de created_at → anotado no teste da linha do tempo com usleep(1100000)
