...
- **Вехи**: M0–M5 сданы (голосовые сообщения, звонки 1v1, голосовые комнаты, стриминг); M6a — виртуальная экономика: кошелёк (баланс/журнал, MySQL единственный источник правды), подарки с долей стримера, мобильное пополнение IAP (App Store / Google Play / Huawei); M6b — платёжные каналы: скелет зачисления пополнения (проверка подписи колбэка WeChat/Alipay/Stripe, серверные цены, идемпотентное зачисление; вывод и сверка сданы); M6c — CDN-хранилище: провайдеры настраиваются из админ-панели (S3-совместимо: AWS S3 / Cloudflare R2 / Aliyun OSS / Tencent COS / Backblaze B2); изображения/голос/файлы раздаются через объектное хранилище + CDN
