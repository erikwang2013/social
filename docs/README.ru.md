...
Монорепозиторий многоязычной социальной платформы: текстово-графическое сообщество + мгновенные сообщения + стримы/голос + виртуальная экономика; связка PHP + Rust, три нативных клиента.

## Талисман проекта

<img src="diagrams/mascot.svg" width="140" alt="Bub — mascot">

**Bub** · пузырь сообщения. Мельчайшая единица платформы: посты, голосовые сообщения, данмаку стримов и подарки отправляются именно в нём; он улыбается, машет рукой, а хвостик слева внизу указывает, куда летит сообщение.

Векторный исходник — [`diagrams/mascot.svg`](diagrams/mascot.svg); упрощённый знак [`mascot-mark.svg`](diagrams/mascot-mark.svg) дополнительно поставляется как `favicon.svg` на сайтах service / admin и в мастере установки.
...
![Структура проекта](diagrams/structure.ru.svg)
...
```bash
cd service && vendor/bin/phpunit      # Модульные тесты (229 tests / 660 assertions; кейсы стримов и голоса требуют запущенного Rust gRPC-сервиса)
cd admin   && vendor/bin/phpunit      # Модульные тесты админки (98 tests / 340 assertions)
cd infrastructure && cargo test --workspace   # Rust, 17 crates (204 tests)
DB_PASS='' php tests/api/run.php      # Автотесты API (116 кейсов; нужны запущенные admin :8791 / service :8788)
cd tests/e2e && npx playwright test   # UI end-to-end (41 кейс)

cd service                            # чёрные ящики E2E ниже требуют запущенного service (:8788/:8789) + Redis
php tests/im_e2e.php                  # IM: обмен по WS двух пользователей / прочтение / отзыв / офлайн-очередь
php tests/voice_e2e.php               # Голос: версии API / голосовые сообщения / сигналинг звонков 1v1 / голосовые комнаты
php tests/live_e2e.php                # Стрим: открытие / вход / данмаку / микрофоны / закрытие
php tests/wallet_e2e.php              # Кошелёк: баланс / пополнение / подарки / доля стримера
php tests/payment_e2e.php             # Платежи: заказ / проверка и зачисление по колбэку / идемпотентность / сверка суммы
php tests/storage_e2e.php             # Хранилище: загрузка изображений / local и s3 / префикс URL совпадает с активным провайдером
