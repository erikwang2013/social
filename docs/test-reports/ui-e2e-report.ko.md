...
- 테스트 케이스 총 **41**개, 통과 **41**, 실패 **0**, 차단 표시 **1**
...
| admin | 배치 작업 `/admin/user/batch/status`(일괄 활성화 + 빈 ids 422), 내보내기 `/admin/export/excel`(xlsx 파일 헤더 검증), 비밀번호 변경 `/admin/profile/password`(이전 비밀번호 누락 422) | 3 |
| service | `/`(iframe 컨테이너), `/health`, `/apidoc`(apidoc/index.html로 리다이렉트), 로그인 없이 보호 엔드포인트 접근 401 | 4 |
| service | 회원가입/로그인/로그아웃, 프로필(GET/PUT `/api/v1/me`), 게시글/타임라인/상세, 좋아요/좋아요 취소, 댓글, 팔로우/언팔로우/관계/팔로워/팔로잉 목록, 알림(목록/안 읽음 수/하나 읽음 표시/전체 읽음 처리) | 10 |
...
2. **service 홈페이지 `/`에 명시적 라우트 필요**: webman-framework v2.2.4의 기본 라우팅은 더 이상 `/`를
   `IndexController@index`로 해석하지 않음(루트 경로에서 404를 일으켜 홈페이지 케이스가 실패한 적 있음). `service/config/route.php`에
   `Route::get('/', ...)`를 명시적으로 등록하여 수정됨; service 재시작 후 적용.
3. **admin 캡차 Imagick 호환성**: 이 머신의 Imagick 빌드에는 `Imagick::RESOURCETYPE_PIXELS` 상수가 없어서,
   `auto` 드라이버가 잘못 ImagickDriver를 선택하고 generate 500을 일으킴(`admin/config/poster.php`는 이제 상수 존재 여부에 따라
   gd로 폴백함; admin 재시작 필요).
4. **admin 캡차 GD 메모리**: `GdDriver`가 큰 이미지(배경 5472x3648)를 디코딩하고 `memory_limit 128M` 상태에서
...
5. **캡차 유형 무작위**: generate가 셋 중 하나를 선택하며, click/rotate는 풀이 가능한 데이터를 노출하지 않아 slider만 자동 통과 가능(최대 12회 재시도).
6. **데이터베이스 root 빈 비밀번호**: 로컬 테스트 환경의 MySQL이 root/빈 비밀번호로 제공되며, 두 앱의 `.env` 기본값이 일치함.
7. **apps/ 모바일**: android/harmonyos/ios는 실행 가능한 웹 UI가 없어 브라우저 E2E에 포함하지 않음.

## 결론

admin 로그인(슬라이더 캡차 포함)과 관리 엔드포인트 22개, service 사용자 측 전 흐름 케이스 19개가 모두 통과함
(이번 실행에 6개 케이스 추가: admin 일괄 활성화/Excel 내보내기/비밀번호 변경 검증, service 미로그인 401/언팔로우/알림 하나 읽음 표시).
실제 결함 2개 수정: service 루트 경로 404(명시적 라우트 추가), admin 캡차 generate 500
(Imagick 상수 누락 → GD로 폴백, 설정에 포함되어 있고 재시작 후 적용).
