...
বহুভাষিক সোশ্যাল প্ল্যাটফর্ম মনোরিপো: টেক্সট/ইমেজ কমিউনিটি + ইনস্ট্যান্ট মেসেজিং + লাইভ/ভয়েস + ভার্চুয়াল ইকোনমি; PHP + Rust দুই স্তর, তিনটি নেটিভ ক্লায়েন্ট।

## প্রকল্পের মাসকট

<img src="diagrams/mascot.svg" width="140" alt="Bub — mascot">

**Bub** · একটি বার্তা-বুদবুদ। প্ল্যাটফর্মের ক্ষুদ্রতম একক: পোস্ট, ভয়েস মেসেজ, লাইভ দানমাকু আর উপহার — সবই একটি বুদবুদে ভরে বেরিয়ে যায়; এটি হাসে, হাত নাড়ে, আর নিচে-বাঁয়ে তার লেজ দেখায় বার্তা কোথায় যাচ্ছে।

ভেক্টর সোর্স: [`diagrams/mascot.svg`](diagrams/mascot.svg); সরলীকৃত মার্ক [`mascot-mark.svg`](diagrams/mascot-mark.svg) `favicon.svg` হিসেবে service / admin সাইট ও অ্যাডমিন ইনস্টল উইজার্ডে যুক্ত হয়েছে।
...
- **নিজস্ব মিডিয়া লেয়ার**: mediasoup SFU + coturn TURN, 1v1 ভয়েস কল এবং ভয়েস চ্যাট রুম (৮টি সিট: হোস্ট + ৭টি মাইক) এর মিডিয়া ফরওয়ার্ডিংয়ের জন্য
- **স্টেট লেয়ারিং**: MySQL ব্যবসার তথ্যের উৎস, Redis সেশন / IM / কল / রুমের রিয়েল-টাইম অবস্থার জন্য
- **মাইলস্টোন**: M0–M5 ডেলিভারি সম্পন্ন (ভয়েস মেসেজ, 1v1 কল, ভয়েস চ্যাট রুম, লাইভ স্ট্রিমিং); M6a ভার্চুয়াল অর্থনীতি ডেলিভারি করে: ওয়ালেট (ব্যালেন্স/লেজার, MySQL একমাত্র সত্যের উৎস), গিফট টিপিং ও স্ট্রিমার শেয়ার, মোবাইল IAP রিচার্জ (App Store / Google Play / Huawei); M6b পেমেন্ট চ্যানেল ডেলিভারি করে: টপ-আপ ক্রেডিট স্কেলটন (WeChat/Alipay/Stripe কলব্যাক স্বাক্ষর যাচাই, সার্ভার-সাইড প্রাইসিং, আইডেম্পোটেন্ট ক্রেডিট; উইথড্রয়াল এবং রিকনসিলিয়েশন ডেলিভারড); M6c CDN স্টোরেজ ডেলিভারি করে: প্রোভাইডার অ্যাডমিন প্যানেল থেকে কনফিগারযোগ্য (S3-সামঞ্জস্যপূর্ণ: AWS S3 / Cloudflare R2 / Aliyun OSS / Tencent COS / Backblaze B2), ছবি/ভয়েস/ফাইল অবজেক্ট স্টোরেজ + CDN দিয়ে বিতরণ; M6d অ্যাডমিন রিপোর্ট এবং ড্যাশবোর্ড পরিসংখ্যান ডেলিভারি করে: রিপোর্ট মডিউল (ব্যবহারকারী/পেমেন্ট/উইথড্রয়াল — তারিখ ফিল্টার, মোট, ট্রেন্ড, ডিস্ট্রিবিউশন, Excel এক্সপোর্ট) এবং হোম পেজে প্ল্যাটফর্ম পরিসংখ্যান কার্ড; v1.1 প্রাইমারি-কী গভর্ন্যান্স ডেলিভার করে: bee_live-এর তিনটি MySQL রাইট এখন idgen_rs snowflake স্পষ্ট প্রাইমারি কী ব্যবহার করে, last_insert_id() নির্ভরতা বাদ পড়ে, সংশ্লিষ্ট টেবিল থেকে AUTO_INCREMENT বাদ; v1.1.1–v1.1.6 ডকুমেন্টেশন / CI / স্থিতিশীলতার সংশোধন, নতুন ফিচার নেই
...
![প্রজেক্ট স্ট্রাকচার](diagrams/structure.bn.svg)
...
পূর্বশর্ত: PHP ≥ 8.3 (composer), MySQL, Redis (Docker ঐচ্ছিক, মিডিয়া লেয়ারের জন্য)।

```bash
./install.sh
```

স্ক্রিপ্ট যা করে: `service/` এবং `admin/`-এ একবার করে `composer install` চালায়; `database/install.sql` থেকে ডেটাবেস তৈরি করে (আইডেম্পোটেন্ট, CREATE IF NOT EXISTS); দুটি সার্ভিসের জন্যই `.env` তৈরি করে (র্যান্ডম JWT / APP কী, বিদ্যমান ফাইল কখনো ওভাররাইট করে না); ঐচ্ছিকভাবে মিডিয়া লেয়ার চালু করে (`docker compose up -d` দিয়ে media/sfu এবং coturn, `--skip-media` দিয়ে বাদ দেওয়া); শেষে প্রতিটি সার্ভিসের চালু করার কমান্ড এবং অ্যাক্সেস URL প্রিন্ট করে।
...
3. এনভায়রনমেন্ট কনফিগার করুন: `service/.env.example` এবং `admin/.env.example` কে `.env`-এ কপি করুন, DB / Redis / JWT / APP কী পূরণ করুন (প্রোডাকশনে সবসময় র্যান্ডম কী ব্যবহার করুন)।
...
5. মিডিয়া লেয়ার চালু করুন (ঐচ্ছিক):
...
```bash
cd service && vendor/bin/phpunit      # ইউনিট টেস্ট (229 tests / 660 assertions; লাইভ ও ভয়েস কেসের জন্য Rust gRPC সেবা চালু থাকতে হবে)
cd admin   && vendor/bin/phpunit      # অ্যাডমিন ইউনিট টেস্ট (98 tests / 340 assertions)
cd infrastructure && cargo test --workspace   # Rust 17 crates (204 tests)
DB_PASS='' php tests/api/run.php      # API অটোমেশন (116 কেস; admin :8791 / service :8788 চালু থাকতে হবে)
cd tests/e2e && npx playwright test   # UI এন্ড-টু-এন্ড (41 কেস)

cd service                            # নিচের ব্ল্যাক-বক্স E2E-এর জন্য service চালু (:8788/:8789) + Redis প্রয়োজন
php tests/im_e2e.php                  # IM: দুই ব্যবহারকারীর WS পাঠানো/পাওয়া / পড়া / প্রত্যাহার / অফলাইন কিউ
php tests/voice_e2e.php               # ভয়েস: API ভার্সনিং / ভয়েস মেসেজ / 1v1 কল সিগন্যালিং / ভয়েস রুম
php tests/live_e2e.php                # লাইভ: শুরু / প্রবেশ / দানমাকু / মাইক ওপর-নিচ / বন্ধ
php tests/wallet_e2e.php              # ওয়ালেট: ব্যালেন্স / রিচার্জ / উপহার / স্ট্রিমারের অংশ
php tests/payment_e2e.php             # পেমেন্ট: অর্ডার / কলব্যাক যাচাই ও জমা / আইডেম্পোটেন্সি / পরিমাণ যাচাই
php tests/storage_e2e.php             # স্টোরেজ: ছবি আপলোড / local ও s3 / URL প্রিফিক্স সক্রিয় প্রোভাইডারের সাথে মিল
